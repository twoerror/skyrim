...
public class TESAlignmentValues {
	public static final AlignmentBonus MARRIAGE_BONUS = new AlignmentBonus(5.0f, "tes.alignment.marriage");
	public static final AlignmentBonus VINEYARD_STEAL_PENALTY = new AlignmentBonus(-1.0f, "tes.alignment.vineyardSteal");
	public static final AlignmentBonus PICKPOCKET_PENALTY = new AlignmentBonus(-1.0f, "tes.alignment.pickpocket");
	...
	private TESAlignmentValues() {
	}

	public static AlignmentBonus createMiniquestBonus(float alignment) {
		return new AlignmentBonus(alignment, "tes.alignment.miniQuest");
	}

	public static AlignmentBonus createPledgePenalty(float alignment) {
		return new AlignmentBonus(alignment, "tes.alignment.breakPledge");
	}
	...
	public static void notifyAlignmentNotHighEnough(ICommandSender entityplayer, float alignmentRequired, TESFaction faction) {
		...
		entityplayer.addChatMessage(new ChatComponentTranslation("tes.chat.insufficientAlignment", componentAlignReq, faction.factionName()));
	}
	...
	public static float parseDisplayedAlign(String alignmentText) {
		...
		try {
			...
			TESLog.getLogger().error("Could not parse alignment value from display string {}", alignmentText1);
			e.printStackTrace();
			return 0.0f;
		}
	}
	...
	private static void setupDecimalFormat(DecimalFormat dFormat) {
		...
		String decimalSeparator = StatCollector.translateToLocal("tes.alignment.decimal_separator_char");
		if (decimalSeparator.length() == 1) {
			decimalSeparatorChar = decimalSeparator.charAt(0);
		}
		if ((groupSeparator = StatCollector.translateToLocal("tes.alignment.group_separator_char")).length() == 1) {
