...
public class TESSpawnDamping {
	...
	private static File getDataFile() {
		return new File(TESLevelData.getOrCreateTESDir(), "spawn_damping.dat");
	}

	public static int getNPCSpawnCap(World world) {
		return getSpawnCap(TYPE_NPC, TESDimension.GAME_OF_THRONES.getSpawnCap(), world);
	}
	...
	public static void loadAll() {
		try {
			...
			NBTTagCompound spawnData = TESLevelData.loadNBTFromFile(datFile);
			SPAWN_DAMPING.clear();
			...
			FMLLog.severe("Error loading TES spawn damping");
			e.printStackTrace();
		}
	}
	...
	public static void saveAll() {
		try {
			...
			TESLevelData.saveNBTToFile(datFile, spawnData);
			needsSave = false;
		} catch (Exception e) {
			FMLLog.severe("Error saving TES spawn damping");
			e.printStackTrace();
		}
	}
