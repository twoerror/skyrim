...
import tes.common.world.TESChunkProvider;
...
public class TESMapGenCaves extends MapGenBase {
	private TESChunkProvider.ChunkFlags chunkFlags;

	public static boolean isTerrainBlock(Block block, BiomeGenBase biome) {
		return block == biome.topBlock || block == biome.fillerBlock || block == Blocks.grass || block == Blocks.dirt || block == Blocks.sand || block == TESBlocks.whiteSand || block == Blocks.gravel || block == TESBlocks.mudGrass || block == TESBlocks.mud || block == TESBlocks.dirtPath || block == Blocks.stone || block == TESBlocks.rock || block == Blocks.sandstone || block == TESBlocks.redSandstone || block == TESBlocks.whiteSandstone || block == TESBlocks.asshaiDirt || block == TESBlocks.basaltGravel;
	}

	private void digBlock(Block[] blockArray, int index, int xzIndex, int j, TESBiome biome, boolean cutSurface) {
		int roadDepth;
		int j1;
							...
							for (var45 = var56; var45 < var40; ++var45) {
								...
								for (int var50 = var57; var50 <= var38 - 1; ++var50) {
									...
									if (var51 > -0.7 && var59 * var59 + var51 * var51 + var46 * var46 < 1.0) {
										TESBiome biome = (TESBiome) TESCrashHandler.getBiomeGenForCoords(worldObj, var42 + par3 * 16, var45 + par4 * 16);
	...
	public void setChunkFlags(TESChunkProvider.ChunkFlags chunkFlags) {
