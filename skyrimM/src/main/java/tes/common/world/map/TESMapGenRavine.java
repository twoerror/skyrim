...
public class TESMapGenRavine extends MapGenRavine {
	...
	public void digBlock(Block[] data, int index, int i, int j, int k, int chunkX, int chunkZ, boolean topBlock) {
		BiomeGenBase biome = TESCrashHandler.getBiomeGenForCoords(worldObj, i + chunkX * 16, k + chunkZ * 16);
		Block top = biome.topBlock;
		Block filler = biome.fillerBlock;
		Block block = data[index];
		if (TESMapGenCaves.isTerrainBlock(block, biome)) {
			if (j < 10) {
	...
	public void func_151538_a(World world, int i, int k, int chunkX, int chunkZ, Block[] blocks) {
		TESCrashHandler.getBiomeGenForCoords(worldObj, chunkX * 16, chunkZ * 16);
		if (rand.nextBoolean()) {
