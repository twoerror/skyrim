...
public class TESGenLayerRemoveMapRivers extends TESGenLayer {
	private final TESDimension dimension;

	public TESGenLayerRemoveMapRivers(long l, TESGenLayer biomes, TESDimension dim) {
		super(l);
		tesParent = biomes;
		dimension = dim;
	}
	...
	public int[] getInts(World world, int i, int k, int xSize, int zSize) {
		int[] biomes = tesParent.getInts(world, i - 4, k - 4, xSize + 4 * 2, zSize + 4 * 2);
		...
		for (int k1 = 0; k1 < zSize; ++k1) {
			for (int i1 = 0; i1 < xSize; ++i1) {
				...
				if (biomeID == TESBiome.river.biomeID) {
					int replaceID = -1;
					for (int range = 1; range <= 4; ++range) {
						...
						for (int k2 = k1 - range; k2 <= k1 + range; ++k2) {
							for (int i2 = i1 - range; i2 <= i1 + range; ++i2) {
								TESBiome subBiome;
								if (Math.abs(i2 - i1) != range && Math.abs(k2 - k1) != range || (subBiome = dimension.getBiomeList()[subBiomeID = biomes[i2 + 4 + (k2 + 4) * (xSize + 4 * 2)]]) == TESBiome.river) {
									continue;
								}
					...
					if (replaceID == -1) {
						FMLLog.warning("WARNING! TES map generation failed to replace map river at %d, %d", i, k);
						ints[i1 + k1 * xSize] = 0;
						continue;
					}
