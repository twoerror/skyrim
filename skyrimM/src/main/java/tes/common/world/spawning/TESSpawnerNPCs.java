...
public class TESSpawnerNPCs {
	...
	private static int countNPCs(World world) {
		...
		try {
			for (Object element : world.loadedEntityList) {
				...
				int spawnCountValue = ((TESEntityNPC) entity).getSpawnCountValue();
	...
	private static TESSpawnEntry.Instance getRandomSpawnListEntry(World world, int i, int k) {
		TESBiomeSpawnList spawnlist = null;
		BiomeGenBase biome = TESCrashHandler.getBiomeGenForCoords(world, i, k);
		if (biome instanceof TESBiome && world.provider instanceof TESWorldProvider) {
			TESBiome tesbiome = (TESBiome) biome;
			TESWorldChunkManager worldChunkMgr = (TESWorldChunkManager) world.provider.worldChunkMgr;
			worldChunkMgr.getBiomeVariantAt(i, k);
			spawnlist = tesbiome.getNPCSpawnList();
		}
		if (spawnlist != null) {
	...
	public static void performSpawning(World world) {
		int interval = TESConfig.mobSpawnInterval;
		if (interval > 0) {
			int ticks = 0;
		...
		int maxSpawnCount = TESSpawnDamping.getNPCSpawnCap(world) * ELIGIBLE_SPAWN_CHUNKS.size() / 196;
			...
			for (int c = 0; c < cycles; ++c) {
				...
				for (ChunkCoordIntPair chunkCoords : shuffled) {
					...
					for (int l = 0; l < groups; ++l) {
						...
						TESSpawnEntry.Instance spawnEntryInstance = getRandomSpawnListEntry(world, i1, k1);
						if (spawnEntryInstance == null) {
							continue;
						}
						TESSpawnEntry spawnEntry = spawnEntryInstance.getSpawnEntry();
						...
						for (int a = 0; a < attempts; ++a) {
							...
							if (entity instanceof TESEntityNPC && isConquestSpawn) {
							...
							if (c > 0 && (totalSpawnCount += entity instanceof TESEntityNPC ? ((TESEntityNPC) entity).getSpawnCountValue() : 1) > maxSpawnCount) {
								break block2;
							}
							spawned++;
