...
public class TESWorldGenPalm extends WorldGenAbstractTree {
	...
	public TESWorldGenPalm(boolean flag, Block b, int m, Block b1, int m1) {
		super(flag);
		woodBlock = b;
		woodMeta = m;
		leafBlock = b1;
		...
		for (int l = 0; l < height; ++l) {
			...
			if (hasDates && l == height - 3) {
				for (int d = 0; d < 4; ++d) {
					...
					setBlockAndNotifyAdequately(world, trunkX + dir.getOpposite().offsetX, j + l, trunkZ + dir.getOpposite().offsetZ, TESBlocks.date, d);
				}
			}
	...
	public TESWorldGenPalm setDates() {
		hasDates = true;
		return this;
	}

	public TESWorldGenPalm setMinMaxHeight(int min, int max) {
		minHeight = min;
		maxHeight = max;
		return this;
	}
