...
public class TESWorldGenGiantTrees extends WorldGenAbstractTree {
	private final Block woodID;
	private final int woodMeta;
	private final Block leafID;
	private final int leafMeta;

	public TESWorldGenGiantTrees(boolean flag, Block i, int j, Block k, int l) {
		super(flag);
		woodID = i;
		woodMeta = j;
		leafID = k;
		...
		for (int j1 = 0; j1 < height; ++j1) {
			...
			for (int i1 = i5 - width; i1 <= i5 + width; ++i1) {
				for (int k1 = k5 - width; k1 <= k5 + width; ++k1) {
					...
					for (int j2 = j - 1; !TES.isOpaque(world, i1, j2, k1) && j2 >= 0 && Math.abs(j2 - j) <= 6 + random.nextInt(5); --j2) {
