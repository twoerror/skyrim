...
public class TESWorldGenAsshaiMoss extends WorldGenerator {
	@Override
		...
		for (int l = 0; l <= numberOfMoss; ++l) {
			...
			for (int i3 = i1; i3 <= i2; ++i3) {
				...
				for (int k3 = k1; k3 <= k2; ++k3) {
					...
					if (j1 != j || d9 * d9 + (d10 = (k3 + 0.5 - d6) / (d8 / 2.0)) * d10 >= 1.0 || world.getBlockMetadata(i3, j1 - 1, k3) != 0 || !world.isAirBlock(i3, j1, k3)) {
						continue;
					}
					world.setBlock(i3, j1, k3, TESBlocks.asshaiMoss, 0, 2);
				}
			}
		}
		return true;
