...
public class TESWorldGenLarch extends WorldGenAbstractTree {
	public TESWorldGenLarch(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if (j >= 1 && j + height + 1 <= 256) {
			...
			boolean isSoil = soil.canSustainPlant(world, i, j - 1, k, ForgeDirection.UP, (IPlantable) TESBlocks.sapling3);
			if (isSoil && j < 256 - height - 1) {
				...
				for (int leafLayer = 0; leafLayer <= leafStart; ++leafLayer) {
					...
					for (int i1 = i - leafRange; i1 <= i + leafRange; ++i1) {
						...
						for (int k1 = k - leafRange; k1 <= k + leafRange; ++k1) {
							...
							setBlockAndNotifyAdequately(world, i1, j1, k1, TESBlocks.leaves3, 1);
						}
					}
				...
				for (j1 = 0; j1 < height - trunkTop; ++j1) {
					...
					setBlockAndNotifyAdequately(world, i, j + j1, k, TESBlocks.wood3, 1);
				}
				return true;
			}
		}
