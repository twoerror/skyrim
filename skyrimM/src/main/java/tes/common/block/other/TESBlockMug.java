...
public class TESBlockMug extends BlockContainer {
	public TESBlockMug() {
		this(3.0f, 8.0f);
	}

	protected TESBlockMug(float f, float f1) {
	...
	private static ItemStack getMugItem(IBlockAccess world, int i, int j, int k) {
		...
		if (tileentity instanceof TESTileEntityMug) {
			TESTileEntityMug mug = (TESTileEntityMug) tileentity;
			return mug.getMugItem();
		}
		return new ItemStack(TESItems.mug);
	}

	public static void setMugItem(IBlockAccess world, int i, int j, int k, ItemStack itemstack, TESItemMug.Vessel vessel) {
		TileEntity te = world.getTileEntity(i, j, k);
		if (te instanceof TESTileEntityMug) {
			TESTileEntityMug mug = (TESTileEntityMug) te;
	...
	public ArrayList<ItemStack> getDrops(World world, int i, int j, int k, int meta, int fortune) {
		...
		if ((meta & 4) == 0) {
			...
			TESTileEntityMug mug = (TESTileEntityMug) world.getTileEntity(i, j, k);
			if (mug != null) {
				drops.add(itemstack);
			}
		}
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		...
		if (tileentity instanceof TESTileEntityMug) {
			...
			if (!mug.isEmpty() && TESItemMug.isItemEmptyDrink(itemstack)) {
				ItemStack takenDrink = mugItem.copy();
				TESItemMug.Vessel v = TESItemMug.getVessel(itemstack);
				TESItemMug.setVessel(takenDrink, v, true);
			...
			if (mug.isEmpty() && TESItemMug.isItemFullDrink(itemstack)) {
				ItemStack emptyMug = TESItemMug.getVessel(itemstack).getEmptyVessel();
			...
			if (!mug.isEmpty()) {
				if (itemstack != null && itemstack.getItem() instanceof TESItemBottlePoison && mug.canPoisonMug()) {
					if (!world.isRemote) {
				...
				ItemStack equivalentDrink = TESItemMug.getEquivalentDrink(mugItem);
				Item eqItem = equivalentDrink.getItem();
				boolean canDrink = eqItem instanceof TESItemMug && ((TESItemMug) eqItem).canPlayerDrink(entityplayer);
				if (canDrink) {
