...
public class TESBlockSpawnerChest extends BlockChest {
	private static boolean dropChestItems = true;

	private final Block chestModel;

	public TESBlockSpawnerChest(Block block) {
		super(0);
		chestModel = block;
	...
	private static void spawnEntity(World world, int i, int j, int k) {
		...
		if (!(tileentity instanceof TESTileEntitySpawnerChest)) {
			return;
		}
		Entity entity = ((TESTileEntitySpawnerChest) tileentity).createMob();
		...
		if (!world.isRemote) {
			...
			if (entityliving instanceof TESEntityNPC) {
				((TESEntityNPC) entityliving).setNPCPersistent(true);
			}
			world.spawnEntityInWorld(entityliving);
			world.playSoundAtEntity(entityliving, "tes:wraith.spawn", 1.0f, 0.7f + world.rand.nextFloat() * 0.6f);
			if (entityliving instanceof TESEntityBarrowWight) {
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TESTileEntitySpawnerChest();
	}
