...
import tes.common.tileentity.TESTileEntitySarbacaneTrap;
...
public class TESBlockSarbacaneTrap extends BlockContainer {
	...
	public TESBlockSarbacaneTrap(Block block, int meta) {
		super(Material.rock);
	...
	public void breakBlock(World world, int i, int j, int k, Block block, int meta) {
		...
		if (trap != null) {
			TES.dropContainerItems(trap, world, i, j, k);
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TESTileEntitySarbacaneTrap();
	}
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		if (!world.isRemote) {
			entityplayer.openGui(TES.instance, TESGuiId.DISPENSER.ordinal(), world, i, j, k);
		}
		return true;
	}
