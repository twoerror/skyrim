...
public class TESBlockGrapevine extends Block implements IPlantable, IGrowable {
	...
	public TESBlockGrapevine(boolean grapes) {
	...
	public static boolean isFullGrownGrapes(Block block, int meta) {
		return block instanceof TESBlockGrapevine && ((TESBlockGrapevine) block).hasGrapes && meta >= 7;
	}

	public static void setHoeing(boolean hoeing) {
		TESBlockGrapevine.hoeing = hoeing;
	}
	...
	public boolean canPlaceBlockAt(World world, int i, int j, int k) {
		...
		return below.isSideSolid(world, i, j - 1, k, ForgeDirection.UP) || below.canSustainPlant(world, i, j, k, ForgeDirection.UP, this) || below instanceof TESBlockGrapevine;
	}
	...
	private boolean checkCanStay(World world, int i, int j, int k) {
		if (!canBlockStay(world, i, j, k)) {
			...
			if (hasGrapes) {
				world.setBlock(i, j, k, TESBlocks.grapevine, 0, 3);
	...
	public int getRenderType() {
		if (hasGrapes) {
			return TES.proxy.getGrapevineRenderID();
		}
		return 0;
	}
	...
	public void registerBlockIcons(IIconRegister iconregister) {
		postIcon = hasGrapes ? TESBlocks.grapevine.getIcon(0, 0) : iconregister.registerIcon(getTextureName());
		if (hasGrapes) {
	...
	public boolean removedByPlayer(World world, EntityPlayer entityplayer, int i, int j, int k, boolean willHarvest) {
		if (hasGrapes && entityplayer != null) {
			return world.setBlock(i, j, k, TESBlocks.grapevine, 0, 3);
		}
	...
	public boolean shouldSideBeRendered(IBlockAccess world, int i, int j, int k, int side) {
		if (hasGrapes) {
			...
			if (block instanceof TESBlockGrapevine && ((TESBlockGrapevine) block).hasGrapes && meta == metaThis && (side == 0 || side == 1)) {
				return false;
			}
		}
