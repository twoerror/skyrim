...
import tes.common.entity.other.inanimate.TESEntityFallingFireJar;
...
public class TESBlockWildFireJar extends BlockFalling {
	...
	public static void setExplodeOnAdded(boolean explodeOnAdded) {
		TESBlockWildFireJar.explodeOnAdded = explodeOnAdded;
	}

	public static void setRenderingStage(int renderingStage) {
		TESBlockWildFireJar.renderingStage = renderingStage;
	}
	...
	public static void explode(World world, int i, int j, int k) {
		if (!world.isRemote) {
			...
			for (int l = 0; l < 64; ++l) {
				...
				world.setBlock(i1, j1, k1, TESBlocks.wildFire, 0, 3);
			}
		}
	}
	...
	public int getRenderType() {
		return TES.proxy.getWildFireJarRenderID();
	}
	...
	public void updateTick(World world, int i, int j, int k, Random random) {
		...
		if (world.getBlock(i, j, k) == this && !world.isRemote && func_149831_e(world, i, j - 1, k) && j >= 0 && world.checkChunksExist(i - (b0 = 32), j - b0, k - b0, i + b0, j + b0, k + b0)) {
			TESEntityFallingFireJar falling = new TESEntityFallingFireJar(world, i + 0.5, j + 0.5, k + 0.5, this, world.getBlockMetadata(i, j, k));
