...
import tes.common.item.other.TESItemKebabStand;
...
public class TESBlockKebabStand extends BlockContainer {
	private final String standTextureName;

	public TESBlockKebabStand(String s) {
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TESTileEntityKebabStand();
	}
	...
	public ArrayList<ItemStack> getDrops(World world, int i, int j, int k, int meta, int fortune) {
		...
		if ((meta & 8) == 0) {
			...
			TESTileEntityKebabStand kebabStand = (TESTileEntityKebabStand) world.getTileEntity(i, j, k);
			if (kebabStand != null) {
				drops.add(itemstack);
			}
		}
	...
	private ItemStack getKebabStandDrop(World world, int i, int j, int k) {
		...
		TESTileEntityKebabStand kebabStand = (TESTileEntityKebabStand) world.getTileEntity(i, j, k);
		if (kebabStand != null) {
			TESItemKebabStand.setKebabData(itemstack, kebabStand);
		}
		return itemstack;
	}
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		...
		if (tileentity instanceof TESTileEntityKebabStand) {
			TESTileEntityKebabStand stand = (TESTileEntityKebabStand) tileentity;
			ItemStack heldItem = entityplayer.getHeldItem();
			if (!stand.isCooked() && TESTileEntityKebabStand.isMeat(heldItem)) {
				...
				if (!world.isRemote) {
					...
					if (meat != null) {
						...
						if (wasCooked) {
							TESLevelData.getData(entityplayer).addAchievement(TESAchievement.cookKebab);
						}
					}
				}
				return true;
	...
	public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entity, ItemStack itemstack) {
		...
		TileEntity tileentity = world.getTileEntity(i, j, k);
		if (tileentity instanceof TESTileEntityKebabStand) {
			TESTileEntityKebabStand kebabStand = (TESTileEntityKebabStand) tileentity;
			TESItemKebabStand.loadKebabData(itemstack, kebabStand);
