...
public class TESBlockOven extends BlockContainer {
	...
	public void breakBlock(World world, int i, int j, int k, Block block, int meta) {
		...
		if (oven != null) {
			TES.dropContainerItems(oven, world, i, j, k);
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		if (!world.isRemote) {
			entityplayer.openGui(TES.instance, TESGuiId.OVEN.ordinal(), world, i, j, k);
		}
		return true;
	}
	...
	public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entity, ItemStack itemstack) {
		...
		if (itemstack.hasDisplayName()) {
			((TESTileEntityOven) world.getTileEntity(i, j, k)).setOvenName(itemstack.getDisplayName());
		}
	}
