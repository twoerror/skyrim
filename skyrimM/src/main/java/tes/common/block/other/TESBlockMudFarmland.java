...
public class TESBlockMudFarmland extends BlockFarmland {
	public TESBlockMudFarmland() {
		setHardness(0.6f);
	...
	public IIcon getIcon(int i, int j) {
		...
		return TESBlocks.mud.getBlockTextureFromSide(i);
	}

	@SideOnly(Side.CLIENT)
	@Override
	public Item getItem(World world, int i, int j, int k) {
		return Item.getItemFromBlock(TESBlocks.mud);
	}

	@Override
	public Item getItemDropped(int i, Random random, int j) {
		return TESBlocks.mud.getItemDropped(0, random, j);
	}
	...
	public void onPlantGrow(World world, int i, int j, int k, int sourceX, int sourceY, int sourceZ) {
		world.setBlock(i, j, k, TESBlocks.mud, 0, 2);
	}
