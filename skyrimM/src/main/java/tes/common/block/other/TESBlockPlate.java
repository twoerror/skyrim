...
public class TESBlockPlate extends BlockContainer {
	public static final Block.SoundType SOUND_TYPE_PLATE = new Block.SoundType("tes:plate", 1.0f, 1.0f) {
		@Override
	...
	public static ItemStack getFoodItem(IBlockAccess world, int i, int j, int k) {
		...
		if (tileentity instanceof TESTileEntityPlate) {
			return ((TESTileEntityPlate) tileentity).getFoodItem();
		}
		return null;
	}
	...
	public void breakBlock(World world, int i, int j, int k, Block block, int meta) {
		ItemStack foodItem;
		TileEntity tileentity = world.getTileEntity(i, j, k);
		if (!world.isRemote && tileentity instanceof TESTileEntityPlate && (foodItem = ((TESTileEntityPlate) tileentity).getFoodItem()) != null) {
	...
	public TileEntity createNewTileEntity(World world, int i) {
		return new TESTileEntityPlate();
	}

	public void dropOnePlateItem(TESTileEntityPlate plate) {
		ItemStack item = plate.getFoodItem().copy();
		item.stackSize = 1;
		dropPlateItem(plate, item);
	}

	private void dropPlateItem(TESTileEntityPlate plate, ItemStack itemstack) {
	...
	public int getRenderType() {
		return TES.proxy.getPlateRenderID();
	}
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		...
		if (tileentity instanceof TESTileEntityPlate) {
			...
			if (plateItem == null && TESTileEntityPlate.isValidFoodItem(itemstack)) {
				if (!world.isRemote) {
