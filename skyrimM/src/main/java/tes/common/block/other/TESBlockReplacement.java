...
public class TESBlockReplacement {
	private static boolean initForgeHooks;

	private TESBlockReplacement() {
	}
	...
	private static class Reflect {
		...
		private static void overwriteBlockList(Block oldBlock, Block newBlock) {
			try {
				...
				TESReflection.setFinalField(null, newBlock, field);
			} catch (Exception e) {
				TESReflection.logFailure(e);
			}
		}
		...
		private static void overwriteItemList(Item oldItem, Item newItem) {
			try {
				...
				TESReflection.setFinalField(null, newItem, field);
			} catch (Exception e) {
				TESReflection.logFailure(e);
			}
		}
