...
public class TESBlockSapling8 extends TESBlockSaplingBase {
	public TESBlockSapling8() {
	...
	public void growTree(World world, int i, int j, int k, Random random) {
		...
		int zOffset = 0;
		if (meta == 0) {
			treeGen = TESTreeType.PLUM.create(true, random);
		}
		if (meta == 1) {
			int[] tree2x2;
			int[] tree4x4;
			...
			if (tree5x5 != null) {
				treeGen = TESTreeType.REDWOOD_5.create(true, random);
				trunkNeg = 2;
				trunkPos = 2;
				xOffset = tree5x5[0];
				zOffset = tree5x5[1];
			...
			if (treeGen == null && (tree4x4 = findSaplingSquare(world, i, j, k, this, 1, -1, 2, -2, 1)) != null) {
				treeGen = TESTreeType.REDWOOD_4.create(true, random);
				trunkNeg = 1;
				trunkPos = 2;
				xOffset = tree4x4[0];
				zOffset = tree4x4[1];
			...
			if (treeGen == null && (tree3x3 = findPartyTree(world, i, j, k, this, 1)) != null) {
				treeGen = TESTreeType.REDWOOD_3.create(true, random);
				trunkNeg = 1;
				trunkPos = 1;
				xOffset = tree3x3[0];
				zOffset = tree3x3[1];
			...
			if (treeGen == null && (tree2x2 = findSaplingSquare(world, i, j, k, this, 1, 0, 1, -1, 0)) != null) {
				treeGen = TESTreeType.REDWOOD_2.create(true, random);
				trunkNeg = 0;
				trunkPos = 1;
				xOffset = tree2x2[0];
				zOffset = tree2x2[1];
			...
			if (treeGen == null) {
				...
				xOffset = 0;
				zOffset = 0;
				treeGen = TESTreeType.REDWOOD.create(true, random);
			}
		}
		if (meta == 2) {
			treeGen = TESTreeType.POMEGRANATE.create(true, random);
		}
		if (meta == 3) {
			treeGen = TESTreeType.PALM.create(true, random);
		}
