...
public class TESBlockSapling7 extends TESBlockSaplingBase {
	public TESBlockSapling7() {
	...
	public void growTree(World world, int i, int j, int k, Random random) {
		...
		if (meta == 0) {
			...
			for (i1 = 0; i1 >= -1; --i1) {
				for (k1 = 0; k1 >= -1; --k1) {
					...
					treeGen = TESTreeType.ASPEN_LARGE.create(true, random);
					trunkNeg = 0;
					trunkPos = 1;
					xOffset = i1;
					zOffset = k1;
			...
			if (treeGen == null) {
				...
				xOffset = 0;
				zOffset = 0;
				treeGen = TESTreeType.ASPEN.create(true, random);
			}
		}
		if (meta == 1) {
			int[] partyTree = findPartyTree(world, i, j, k, this, 1);
			if (partyTree != null) {
				treeGen = TESTreeType.ULTHOS_GREEN_OAK_LARGE.create(true, random);
				trunkPos = 1;
				trunkNeg = 1;
				xOffset = partyTree[0];
				zOffset = partyTree[1];
			...
			if (treeGen == null) {
				treeGen = TESTreeType.ULTHOS_GREEN_OAK.create(true, random);
				trunkPos = 0;
				trunkNeg = 0;
				xOffset = 0;
				zOffset = 0;
		...
		if (meta == 2) {
			treeGen = TESTreeType.FOTINIA.create(true, random);
		}
		if (meta == 3) {
			treeGen = TESTreeType.ALMOND.create(true, random);
		}
