...
public class TESContainerPouch extends Container {
	private final int thePouchSlot;
	private final TESInventoryPouch pouchInventory;
	private final int capacity;

	private ItemStack thePouchItem;

	public TESContainerPouch(EntityPlayer entityplayer, int slot) {
		int i;
		int j;
		thePouchSlot = slot;
		thePouchItem = entityplayer.inventory.getStackInSlot(thePouchSlot);
		pouchInventory = new TESInventoryPouch(entityplayer, this, slot);
		capacity = pouchInventory.getSizeInventory();
		int rows = capacity / 9;
		for (i = 0; i < rows; ++i) {
			for (j = 0; j < 9; ++j) {
				addSlotToContainer(new TESSlotPouch(pouchInventory, j + i * 9, 8 + j * 18, 30 + i * 18));
			}
		}
		for (i = 0; i < 3; ++i) {
			for (j = 0; j < 9; ++j) {
	...
	public TESInventoryPouch getPouchInventory() {
