...
public class TESContainerCoinExchange extends Container {
	...
	public TESContainerCoinExchange(EntityPlayer entityplayer, TESEntityNPC npc) {
		theTraderNPC = npc;
	...
	private static boolean isValidCoin(ItemStack item) {
		return item.getItem() == TESItems.coin && !TESPickpoketableHelper.isPickpocketed(item);
	}
			...
			if (!exchanged) {
				...
				if (coin != null && coin.stackSize > 0 && isValidCoin(coin)) {
					...
					if (coinType > 0) {
						...
						exchangeInv.setInventorySlotContents(0, new ItemStack(TESItems.coin, coinsFloor * 4, coinType - 1));
					} else {
						exchangeInv.setInventorySlotContents(0, null);
					}
					if (coinType < TESItemCoin.VALUES.length - 1 && coins >= 4) {
						exchangeInv.setInventorySlotContents(1, new ItemStack(TESItems.coin, coins / 4, coinType + 1));
					} else {
