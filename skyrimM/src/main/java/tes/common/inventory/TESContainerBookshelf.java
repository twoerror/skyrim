...
public class TESContainerBookshelf extends Container {
	private final TESTileEntityBookshelf shelfInv;
	private final int numRows;

	public TESContainerBookshelf(IInventory player, TESTileEntityBookshelf shelf) {
		int j;
		int i;
		shelfInv = shelf;
			...
			for (i = 0; i < 9; ++i) {
				addSlotToContainer(new Slot(shelfInv, i + j * 9, 8 + i * 18, 18 + j * 18) {
					...
					public boolean isItemValid(ItemStack itemstack) {
						return TESTileEntityBookshelf.isBookItem(itemstack);
					}
				});
			}
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer entityplayer, int i) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			} else if (TESTileEntityBookshelf.isBookItem(itemstack)) {
	...
	public TESTileEntityBookshelf getShelfInv() {
		return shelfInv;
	}
}