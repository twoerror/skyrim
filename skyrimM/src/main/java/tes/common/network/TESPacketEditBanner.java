...
public class TESPacketEditBanner implements IMessage {
	...
	public TESPacketEditBanner() {
	}

	public TESPacketEditBanner(TESEntityBanner banner) {
	...
	public static class Handler implements IMessageHandler<TESPacketEditBanner, IMessage> {
		@Override
		public IMessage onMessage(TESPacketEditBanner packet, MessageContext context) {
			TESEntityBanner banner;
			EntityPlayerMP entityplayer = context.getServerHandler().playerEntity;
			World world = entityplayer.worldObj;
			Entity bEntity = world.getEntityByID(packet.bannerID);
			if (bEntity instanceof TESEntityBanner && (banner = (TESEntityBanner) bEntity).canPlayerEditBanner(entityplayer)) {
				...
				if (packet.whitelistSlots != null) {
					for (int index = 0; index < packet.whitelistSlots.length; ++index) {
						...
						List<TESBannerProtection.Permission> decodedPerms = TESBannerWhitelistEntry.static_decodePermBitFlags(perms);
