...
public class TESPacketUpdatePlayerLocations implements IMessage {
	...
	public static class Handler implements IMessageHandler<TESPacketUpdatePlayerLocations, IMessage> {
		@Override
		public IMessage onMessage(TESPacketUpdatePlayerLocations packet, MessageContext context) {
			TES.proxy.clearMapPlayerLocations();
			for (PlayerLocationInfo info : packet.playerLocations) {
				TES.proxy.addMapPlayerLocation(info.getPlayerProfile(), info.getPosX(), info.getPosZ());
			}
			return null;
		}
	}
