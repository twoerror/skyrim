...
import tes.common.block.other.TESBlockKebabStand;
...
public class TESTileEntityKebabStand extends TileEntity implements IInventory {
	...
	private void cookFirstMeat() {
		...
		for (int i = getSizeInventory() - 1; i >= 0; --i) {
			...
			setInventorySlotContents(i, new ItemStack(TESItems.kebab));
			cooked[i] = true;
			break;
		}
	}
	...
	public String getStandTextureName() {
		...
		if (block instanceof TESBlockKebabStand) {
			return ((TESBlockKebabStand) block).getStandTextureName();
		}
		return "";
	}
