...
public class TESTileEntitySarbacaneTrap extends TileEntityDispenser {
	private int fireCooldown;

	@Override
	public String getInventoryName() {
		return hasCustomInventoryName() ? field_146020_a : "tes.container.dartTrap";
	}
	...
	public void updateEntity() {
		...
		if (!worldObj.isRemote) {
			if (fireCooldown > 0) {
				...
				if (slot >= 0 && (itemstack = getStackInSlot(slot)).getItem() instanceof TESItemDart && !worldObj.selectEntitiesWithinAABB(EntityLivingBase.class, getTriggerRange(), TES.selectLivingExceptCreativePlayers()).isEmpty()) {
