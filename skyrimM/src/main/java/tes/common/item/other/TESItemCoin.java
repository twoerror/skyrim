...
public class TESItemCoin extends Item {
	...
	private static int getSingleItemValue(ItemStack itemstack, boolean allowStolen) {
		if (itemstack != null && itemstack.getItem() instanceof TESItemCoin) {
			if (!allowStolen && TESPickpoketableHelper.isPickpocketed(itemstack)) {
				return 0;
			}
	...
	public static void giveCoins(int coins, EntityPlayer entityplayer) {
		...
		for (i = VALUES.length - 1; i >= 0; --i) {
			value = VALUES[i];
			coin = new ItemStack(TESItems.coin, 1, i);
	...
	public void addInformation(ItemStack itemstack, EntityPlayer entityplayer, List list, boolean flag) {
		...
		list.add(EnumChatFormatting.DARK_GRAY + StatCollector.translateToLocal("item.tes:coin.nominal.name") + ": " + VALUES[i]);
	}
