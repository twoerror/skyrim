...
import tes.TES;
import tes.common.TESGuiHandler;
...
public class TESItemPouch extends Item {
	...
	public static IInventory getChestInvAt(EntityPlayer entityplayer, World world, int i, int j, int k) {
		...
		if (block instanceof TESBlockChest) {
			return TESBlockChest.getModChestAt(world, i, j, k);
		}
	...
	public static boolean isHoldingPouch(EntityPlayer entityplayer, int slot) {
		return entityplayer.inventory.getStackInSlot(slot) != null && entityplayer.inventory.getStackInSlot(slot).getItem() instanceof TESItemPouch;
	}
	...
	public void addInformation(ItemStack itemstack, EntityPlayer entityplayer, List list, boolean flag) {
		...
		int slotsFull = 0;
		IInventory pouchInv = new TESInventoryPouch(itemstack);
		...
		list.add(StatCollector.translateToLocalFormatted("item.tes.pouch.slots", slotsFull, slots));
		if (isPouchDyed(itemstack)) {
			list.add(StatCollector.translateToLocal("item.tes.pouch.dyed"));
		}
	}
	...
	public IIcon getIcon(ItemStack itemstack, int pass) {
		...
		if (entityplayer != null && ((container = entityplayer.openContainer) instanceof TESContainerPouch || container instanceof TESContainerChestWithPouch) && itemstack == entityplayer.getHeldItem()) {
			open = true;
		}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
		if (!world.isRemote) {
			entityplayer.openGui(TES.instance, TESGuiId.POUCH.ordinal(), world, entityplayer.inventory.currentItem, 0, 0);
		}
		return itemstack;
	}
	...
	public boolean onItemUseFirst(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int side, float hitX, float hitY, float hitZ) {
		...
		if (chest != null) {
			TESGuiHandler.usePouchOnChest(entityplayer, world, i, j, k, side, itemstack, entityplayer.inventory.currentItem);
			return true;
		}
		return false;
	}
