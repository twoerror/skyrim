...
public abstract class TESItemRugBase extends Item {
	private final String[] rugNames;

	@SideOnly(Side.CLIENT)
	private IIcon[] rugIcons;

	protected TESItemRugBase(String... names) {
		rugNames = names;
	...
	protected abstract TESEntityRugBase createRug(World var1, ItemStack var2);
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int l, float f, float f1, float f2) {
		...
		if (world.getBlock(i1, j1 - 1, k1).isSideSolid(world, i1, j1 - 1, k1, ForgeDirection.UP) && !world.isRemote) {
			TESEntityRugBase rug = createRug(world, itemstack);
