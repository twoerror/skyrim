...
public class TESItemBrandingIron extends Item {
	...
	public static UUID getBrandingPlayer(Entity entity) {
		...
		if (nbt.hasKey("TESBrander")) {
			String s = nbt.getString("TESBrander");
	...
	private static void setBrandingPlayer(Entity entity, UUID player) {
		...
		entity.getEntityData().setString("TESBrander", s);
	}
	...
	public String getItemStackDisplayName(ItemStack itemstack) {
		...
		if (hasBrandName(itemstack)) {
			...
			return StatCollector.translateToLocalFormatted("item.tes.brandingIron.named", name, brandName);
		}
		return StatCollector.translateToLocalFormatted("item.tes.brandingIron.unnamed", name);
	}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
		if (!hasBrandName(itemstack)) {
			entityplayer.openGui(TES.instance, TESGuiId.BRANDING_IRON.ordinal(), world, 0, 0, 0);
		}
		return itemstack;
	}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int side, float f, float f1, float f2) {
		if (hasBrandName(itemstack) && !isHeated(itemstack)) {
			...
			if (te instanceof TileEntityFurnace && ((TileEntityFurnace) te).isBurning() || te instanceof TESTileEntityAlloyForge && ((TESTileEntityAlloyForge) te).isSmelting()) {
				isHotBlock = true;
			} else if (te instanceof TESTileEntityOven && ((TESTileEntityOven) te).isCooking()) {
				isHotBlock = true;
			}
			if (!isHotBlock) {
