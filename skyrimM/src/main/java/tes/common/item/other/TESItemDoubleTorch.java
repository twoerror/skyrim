...
import tes.common.block.other.TESBlockDoubleTorch;
...
public class TESItemDoubleTorch extends Item {
	private final Block torchBlock;

	public TESItemDoubleTorch(Block block) {
		setCreativeTab(TESCreativeTabs.TAB_DECO);
		torchBlock = block;
		((TESBlockDoubleTorch) torchBlock).setTorchItem(this);
	}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int side, float f, float f1, float f2) {
		...
		if (!TESBlockDoubleTorch.canPlaceTorchOn(world, i1, j1 - 1, k1)) {
			return false;
		}
		if (!world.isRemote) {
