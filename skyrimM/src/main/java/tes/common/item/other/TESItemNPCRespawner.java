...
public class TESItemNPCRespawner extends Item {
	public TESItemNPCRespawner() {
		setMaxStackSize(1);
		setCreativeTab(TESCreativeTabs.TAB_MISC);
	}

	private static void placeSpawnerAt(World world, int i, int j, int k) {
		TESEntityNPCRespawner spawner = new TESEntityNPCRespawner(world);
		double f = 0.1;
		double f1 = 1.0 - f;
		List<TESEntityNPCRespawner> entities = world.getEntitiesWithinAABB(TESEntityNPCRespawner.class, AxisAlignedBB.getBoundingBox(i + f, j + f, k + f, i + f1, j + f1, k + f1));
		if (entities.isEmpty()) {
