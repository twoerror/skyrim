...
public class TESRecipePoisonDrinks implements IRecipe {
	@Override
	public ItemStack getCraftingResult(InventoryCrafting inv) {
		...
		block12:
		{
			...
			for (int i = 0; i < inv.getSizeInventory(); ++i) {
				...
				if (TESPoisonedDrinks.canPoison(itemstack)) {
					if (drink != null) {
						return null;
					}
				...
				if (TES.isOreNameEqual(itemstack, "poison")) {
					if (poison != null) {
						return null;
					}
			...
			result = drink.copy();
			TESPoisonedDrinks.setDrinkPoisoned(result, true);
			craftingPlayer = null;
			try {
				Container cwb = null;
		...
		if (craftingPlayer != null) {
			TESPoisonedDrinks.setPoisonerPlayer(result, craftingPlayer);
		} else {
			FMLLog.bigWarning("TES Warning! Poisoned drink was crafted, player could not be found!");
		}
		return result;
	}
