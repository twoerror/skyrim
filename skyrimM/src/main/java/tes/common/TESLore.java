...
public class TESLore {
	...
	private TESLore(String name, String title, String auth, String text, boolean reward) {
		loreName = name;
		loreTitle = title;
		loreAuthor = auth;
		loreText = text;
	...
	public static TESLore getMultiRandomLore(Iterable<LoreCategory> categories, Random random, boolean rewardsOnly) {
		ArrayList<TESLore> allLore = new ArrayList<>();
		for (LoreCategory c : categories) {
			for (TESLore lore : c.getLoreList()) {
	...
	public static void onInit() {
		...
		try {
			...
			if (mc.getSource().isFile()) {
				...
				while (entries.hasMoreElements()) {
					...
					if (s.startsWith(path = "assets/tes/texts/" + TESConfig.languageCode + "/lore/") && s.endsWith(".txt")) {
						...
						try {
							...
							TESLog.getLogger().error("Failed to onInit TES lore {}from zip file", s);
							e.printStackTrace();
						}
					}
				}
			} else {
				File nameBankDir = new File(TES.class.getResource("/assets/tes/texts/" + TESConfig.languageCode + "/lore/").toURI());
				for (File file : nameBankDir.listFiles()) {
					String s = file.getName();
					int i = s.indexOf(".txt");
					if (i < 0) {
						TESLog.getLogger().error("Failed to onInit TES lore {} from MCP folder; name bank files must be in .txt format", s);
					} else {
						try {
							s = s.substring(0, i);
							...
							TESLog.getLogger().error("Failed to onInit TES lore {} from MCP folder", s);
							e.printStackTrace();
						}
					}
				}
			...
			TESLog.getLogger().error("Failed to onInit TES lore");
			e.printStackTrace();
		}
		for (Map.Entry<String, BufferedReader> entry : loreReaders.entrySet()) {
			...
			try {
					...
					if (line.startsWith(CODE_METADATA)) {
						...
						if (metadata.startsWith(CODE_CATEGORY)) {
							...
							while (!categoryString.isEmpty()) {
								...
								TESLog.getLogger().warn("Hummel009: Loading lore {}, no category exists for name {}", loreName, categoryName);
							}
							continue;
						}
				...
				reader.close();
				TESLore lore = new TESLore(loreName, title, author, text.toString(), reward);
				for (LoreCategory category : categories) {
					category.addLore(lore);
				}
			} catch (Exception e) {
				TESLog.getLogger().error("Failed to onInit TES lore: {}", loreName);
				e.printStackTrace();
			}
		}
		for (LoreCategory category : LoreCategory.values()) {
			...
			int numReward = 0;
			for (TESLore lore : category.getLoreList()) {
				if (lore.isRewardable) {
					++numReward;
				}
			}
			TESLog.getLogger().info("Hummel009: Category {} has loaded {} lore texts, of which {} rewardable", category.getCategoryName(), num, numReward);
		}
		if (zip != null) {
			try {
				zip.close();
			...
			block16:
			{
				...
				if (formatted.startsWith("num:")) {
					try {
						...
						TESLog.getLogger().error("Hummel009: Error formatting number {} in text: {}", unformatted, loreName);
						e.printStackTrace();
					}
				} else if (formatted.startsWith("name:")) {
					try {
						String namebank = formatted.substring("name:".length());
						if (!TESNames.nameBankExists(namebank)) {
							TESLog.getLogger().error("Hummel009: No namebank exists for {}!", namebank);
							break block16;
						}
						formatted = TESNames.getRandomName(namebank, random);
					} catch (Exception e) {
						TESLog.getLogger().error("Hummel009: Error formatting name {} in text: {}", unformatted, loreName);
						e.printStackTrace();
					}
					...
					try {
						...
						TESLog.getLogger().error("Hummel009: Error formatting choice {} in text: {}", unformatted, loreName);
						e.printStackTrace();
					}
				}
			}
	...
	public enum LoreCategory {
		...
		private final Collection<TESLore> loreList = new ArrayList<>();
		...
		private void addLore(TESLore lore) {
			loreList.add(lore);
		}

		public Collection<TESLore> getLoreList() {
			return loreList;
		}
