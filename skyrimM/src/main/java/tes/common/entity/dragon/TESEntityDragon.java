...
import tes.client.model.TESModelDragonBodyHelper;
import tes.common.entity.ai.*;
import tes.common.entity.dragon.helper.TESDragonHelper;
import tes.common.entity.dragon.helper.TESDragonLifeStageHelper;
import tes.common.entity.dragon.helper.TESDragonParticleHelper;
import tes.common.entity.dragon.helper.TESDragonReproductionHelper;
...
public class TESEntityDragon extends EntityTameable implements TESBiome.ImmuneToFrost {
	...
	private final TESDragonFlightWaypoint waypoint;
	private final EntityAITasks airTasks;

	private Map<Class<? extends TESDragonHelper>, TESDragonHelper> helpers;

	private TESModelDragonAnimaton animator;
	...
	public TESEntityDragon(World world) {
		super(world);
		waypoint = new TESDragonFlightWaypoint(this);
		airTasks = new EntityAITasks(world != null ? world.theProfiler : null);
		try {
			ReflectionHelper.setPrivateValue(EntityLiving.class, this, new TESModelDragonBodyHelper(this), ENTITYLIVING_BODYHELPER);
		} catch (Exception ex) {
			TESLog.getLogger().warn("Can't override EntityBodyHelper", ex);
		}
		setSize(BASE_WIDTH, BASE_HEIGHT);
		stepHeight = 1;
		tasks.addTask(0, new TESEntityAIDragonCatchOwnerGround(this));
		tasks.addTask(1, new TESEntityAIDragonRideGround(this, 1));
		tasks.addTask(2, new EntityAISwimming(this));
		tasks.addTask(3, aiSit);
		tasks.addTask(4, new TESEntityAIDragonDragonMate(this, 0.6));
		...
		tasks.addTask(8, new TESEntityAIDragonFollowOwner(this, 1, 12, 128));
		tasks.addTask(8, new TESEntityAIDragonPanicChild(this, 1));
		tasks.addTask(9, new EntityAIWander(this, 1));
		tasks.addTask(10, new TESEntityAIDragonWatchIdle(this));
		tasks.addTask(10, new TESEntityAIDragonWatchLiving(this, 16, 0.05f));
		airTasks.addTask(0, new TESEntityAIDragonRideAir(this));
		airTasks.addTask(0, new TESEntityAIDragonLand(this));
		airTasks.addTask(0, new TESEntityAIDragonCatchOwnerAir(this));
		targetTasks.addTask(1, new EntityAIOwnerHurtByTarget(this));
		targetTasks.addTask(2, new EntityAIOwnerHurtTarget(this));
		targetTasks.addTask(3, new EntityAIHurtByTarget(this, false));
		targetTasks.addTask(4, new TESEntityAIDragonHunt(this, EntitySheep.class, 200, false));
		targetTasks.addTask(4, new TESEntityAIDragonHunt(this, EntityPig.class, 200, false));
		targetTasks.addTask(4, new TESEntityAIDragonHunt(this, EntityChicken.class, 200, false));
		isImmuneToFire = true;
	}
	...
	private void addHelper(TESDragonHelper helper) {
		TESLog.getLogger().trace("addHelper({})", helper.getClass().getName());
		if (helpers == null) {
			helpers = new HashMap<>();
		}
	...
	public void applyEntityAttributes() {
		...
		setAttributes();
		for (TESDragonHelper helper : helpers.values()) {
	...
	public void entityInit() {
		...
		addHelper(new TESDragonLifeStageHelper(this));
		addHelper(new TESDragonReproductionHelper(this, INDEX_BREEDER, INDEX_REPRO_COUNT));
		addHelper(new TESDragonParticleHelper(this));
		if (isClient()) {
			animator = new TESModelDragonAnimaton(this);
		}
	}
	...
	public TESDragonFlightWaypoint getWaypoint() {
		return waypoint;
	}
	...
	public void setCanFly(boolean canFly) {
		TESLog.getLogger().trace("setCanFly({})", canFly);
	...
	public void liftOff() {
		TESLog.getLogger().trace("liftOff");
		if (isCanFly()) {
			jump();
			motionY += 0.5;
			inAirTicks += 20;
	...
	private void onUpdateFlyingClient() {
		if (newPosRotationIncrements > 0) {
			...
			double newYaw = TESModelDragonAnimaton.normDeg(newRotationYaw - rotationYaw);
	...
	private void onUpdateFlyingServer() {
		...
		if (!waypoint.isNear()) {
			...
			deltaY = TESModelDragonAnimaton.clamp(deltaY, -speedHoriz, speedHoriz) / 3;
			double motionHypot = Math.hypot(motionX, motionZ) + 1;
			double newYaw = Math.toDegrees(Math.PI * 2 - Math.atan2(deltaX, deltaZ));
			double yawDelta = TESModelDragonAnimaton.normDeg(newYaw - rotationYaw);
			int yawSpeed = 30;
			yawDelta = TESModelDragonAnimaton.clamp(yawDelta, -yawSpeed, yawSpeed);
			yawAdd *= 0.8f;
	...
	public void setMoveSpeedAirHoriz(double airSpeedHorizonal) {
		TESLog.getLogger().trace("setMoveSpeedAirHoriz({})", airSpeedHorizonal);
		this.airSpeedHorizonal = airSpeedHorizonal;
	}

	public void setMoveSpeedAirVert(double airSpeedVertical) {
		TESLog.getLogger().trace("setMoveSpeedAirVert({})", airSpeedVertical);
	...
	public TESModelDragonAnimaton getAnimator() {
		return animator;
	}
	...
	private <T extends TESDragonHelper> T getHelper(Class<T> clazz) {
	...
	public TESDragonLifeStageHelper getLifeStageHelper() {
		return getHelper(TESDragonLifeStageHelper.class);
	}
	...
	private TESDragonParticleHelper getParticleHelper() {
		return getHelper(TESDragonParticleHelper.class);
	}
	...
	public TESDragonReproductionHelper getReproductionHelper() {
		return getHelper(TESDragonReproductionHelper.class);
	}
	...
	private void setRidingPlayer(EntityPlayer player) {
		TESLog.getLogger().trace("setRidingPlayer({})", player.getCommandSenderName());
	...
	private void setSaddled(boolean saddled) {
		TESLog.getLogger().trace("setSaddled({})", saddled);
