...
public class TESDragonReproductionHelper extends TESDragonHelper {
	...
	public TESDragonReproductionHelper(TESEntityDragon dragon, int dataIndexBreeder, int dataIndexReproCount) {
		super(dragon);
	...
	private static String fixChildName(String nameOld) {
		...
		if (!nameOld.equals(nameNew)) {
			TESLog.getLogger().debug("Fixed child name {} -> {}");
		}
		return nameNew;
	}
	...
	public boolean canMateWith(EntityAnimal mate) {
		if (mate == dragon || !dragon.isTamed() || !(mate instanceof TESEntityDragon) || !canReproduce()) {
			return false;
		}
		TESEntityDragon dragonMate = (TESEntityDragon) mate;
	...
	public EntityAgeable createChild(EntityAgeable mate) {
		if (!(mate instanceof TESEntityDragon)) {
			throw new IllegalArgumentException("The mate isn't a dragon");
		}
		TESEntityDragon parent1 = dragon;
		TESEntityDragon parent2 = (TESEntityDragon) mate;
		TESEntityDragon baby = new TESEntityDragon(dragon.worldObj);
	...
	public void setBreederName(String breederName) {
		TESLog.getLogger().trace("setBreederName({})", breederName);
	...
	private void setReproCount(int reproCount) {
		TESLog.getLogger().trace("setReproCount({})", reproCount);
