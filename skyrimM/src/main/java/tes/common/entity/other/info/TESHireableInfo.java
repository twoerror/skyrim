...
import tes.common.network.TESPacketHiredGui;
import tes.common.network.TESPacketHiredInfo;
...
public class TESHireableInfo {
	...
	private TESUnitTradeEntry.PledgeType pledgeType = TESUnitTradeEntry.PledgeType.NONE;
	private Task hiredTask = Task.WARRIOR;

	private TESInventoryNPC hiredInventory;
	...
	public TESHireableInfo(TESEntityNPC npc) {
		theEntity = npc;
	}
	...
	public void commandSwordAttack(EntityLivingBase target) {
		if (TES.canNPCAttackEntity(theEntity, target, true)) {
	...
	public void dismissUnit(boolean isDesertion) {
		if (isDesertion) {
			getHiringPlayer().addChatMessage(new ChatComponentTranslation("tes.hiredNPC.desert", theEntity.getCommandSenderName()));
		} else {
			getHiringPlayer().addChatMessage(new ChatComponentTranslation("tes.hiredNPC.dismiss", theEntity.getCommandSenderName()));
		}
	...
	public TESInventoryNPC getHiredInventory() {
	...
	public String getStatusString() {
		...
		if (hiredTask == Task.WARRIOR) {
			status = inCombat ? StatCollector.translateToLocal("tes.hiredNPC.status.combat") : isHalted() ? StatCollector.translateToLocal("tes.hiredNPC.status.halted") : guardMode ? StatCollector.translateToLocal("tes.hiredNPC.status.guard") : StatCollector.translateToLocal("tes.hiredNPC.status.ready");
		} else if (hiredTask == Task.FARMER) {
			status = guardMode ? StatCollector.translateToLocal("tes.hiredNPC.status.farming") : StatCollector.translateToLocal("tes.hiredNPC.status.following");
		}
		return StatCollector.translateToLocalFormatted("tes.hiredNPC.status", status);
	}
	...
	private void setTask(Task t) {
		...
		if (hiredTask == Task.FARMER) {
			hiredInventory = new TESInventoryNPC("HiredInventory", theEntity, 4);
		}
	}
	...
	public void hireUnit(EntityPlayer entityplayer, boolean setLocation, TESFaction hiringFaction, TESUnitTradeEntry tradeEntry, String squadron, Entity mount) {
		float alignment = tradeEntry.getAlignmentRequired();
		TESUnitTradeEntry.PledgeType pledge = tradeEntry.getPledgeType();
		...
		if (hiringFaction != null && hiringFaction.isPlayableAlignmentFaction()) {
			TESLevelData.getData(entityplayer).getFactionData(hiringFaction).addHire();
		}
		if (mount != null) {
			...
			if (mount instanceof TESEntityNPC) {
				TESEntityNPC hiredMountNPC = (TESEntityNPC) mount;
			...
			if (mount instanceof TESNPCMount && !(mount instanceof TESEntityNPC)) {
				theEntity.setRidingHorse(true);
				TESNPCMount hiredHorse = (TESNPCMount) mount;
				hiredHorse.setBelongsToNPC(true);
				TESMountFunctions.setNavigatorRangeFromNPC(hiredHorse, theEntity);
			}
		}
	}
	...
	public void onDeath() {
		...
		if (!theEntity.worldObj.isRemote && isActive && getHiringPlayer() != null && TESLevelData.getData(hiringPlayer = getHiringPlayer()).getEnableHiredDeathMessages()) {
			hiringPlayer.addChatMessage(new ChatComponentTranslation("tes.hiredNPC.death", theEntity.func_110142_aN().func_151521_b()));
		}
	...
	public void onKillEntity(EntityLivingBase target) {
		if (!theEntity.worldObj.isRemote && isActive) {
			...
			if (hiredTask == Task.WARRIOR) {
				...
				int addXP = 0;
				TESFaction unitFaction = theEntity.getFaction();
				if (target instanceof EntityPlayer) {
					wasEnemy = TESLevelData.getData((EntityPlayer) target).getAlignment(unitFaction) < 0.0f;
				} else {
					TESFaction targetFaction = TES.getNPCFaction(target);
					if (targetFaction.isBadRelation(unitFaction) || unitFaction == TESFaction.UNALIGNED && targetFaction != TESFaction.UNALIGNED) {
						wasEnemy = true;
						addXP = 1;
					}
				}
	...
	private void onLevelUp() {
		...
		if (hirer != null) {
			hirer.addChatMessage(new ChatComponentTranslation("tes.hiredNPC.levelUp", theEntity.getCommandSenderName(), xpLevel));
		}
	...
	public void onUpdate() {
		if (!theEntity.worldObj.isRemote) {
			...
			if (hasHiringRequirements() && isActive && (entityplayer = getHiringPlayer()) != null) {
				TESFaction fac = theEntity.getFaction();
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if (data != null) {
			...
			if (data.hasKey("PledgeType")) {
				...
				pledgeType = TESUnitTradeEntry.PledgeType.forID(pledgeID);
			}
	...
	public void receiveBasicData(TESPacketHiredInfo packet) {
	...
	public void receiveClientPacket(TESPacketHiredGui packet) {
	...
	public void sendBasicData(EntityPlayerMP entityplayer) {
		IMessage packet = new TESPacketHiredInfo(theEntity.getEntityId(), hiringPlayerUUID, hiredTask, hiredSquadron, xpLevel);
	...
	public void sendClientPacket(boolean shouldOpenGui) {
		...
		TESPacketHiredGui packet = new TESPacketHiredGui(theEntity.getEntityId(), shouldOpenGui);
		...
		TESPacketHandler.NETWORK_WRAPPER.sendTo(packet, (EntityPlayerMP) getHiringPlayer());
		if (shouldOpenGui) {
			isGuiOpen = true;
		}
	}
