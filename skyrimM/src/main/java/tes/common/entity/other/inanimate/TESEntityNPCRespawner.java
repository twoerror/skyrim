...
import tes.common.network.TESPacketNPCRespawner;
...
public class TESEntityNPCRespawner extends Entity {
	...
	public TESEntityNPCRespawner(World world) {
		super(world);
		setSize(1.0f, 1.0f);
		spawnerSpin = rand.nextFloat() * 360.0f;
	}

	public static boolean isSpawnBlocked(Entity entity, TESFaction spawnFaction) {
		int k;
		int j;
		int range;
		...
		List<TESEntityNPCRespawner> spawners = world.getEntitiesWithinAABB(TESEntityNPCRespawner.class, searchBB);
		if (!spawners.isEmpty()) {
			for (TESEntityNPCRespawner obj : spawners) {
	...
	public static boolean isSpawnBlocked(TESEntityNPC npc) {
	...
	public ItemStack getPickedResult(MovingObjectPosition target) {
		return new ItemStack(TESItems.npcRespawner);
	}
	...
	public void handleHealthUpdate(byte b) {
		if (b == 16) {
			for (int l = 0; l < 16; ++l) {
				...
				worldObj.spawnParticle("iconcrack_" + Item.getIdFromItem(TESItems.npcRespawner), d, d1, d2, 0.0, 0.0, 0.0);
			}
		} else {
	...
	public boolean interactFirst(EntityPlayer entityplayer) {
		if (entityplayer.capabilities.isCreativeMode) {
			if (!worldObj.isRemote) {
				IMessage packet = new TESPacketNPCRespawner(this);
	...
	private boolean isEnemySpawnBlocked(TESFaction spawnFaction) {
		TESFaction faction1;
		TESFaction faction2;
		return spawnClass1 != null && (faction1 = ((TESEntityNPC) EntityList.createEntityByName(TESEntityRegistry.getStringFromClass(spawnClass1), worldObj)).getFaction()) != null && faction1.isBadRelation(spawnFaction) || spawnClass2 != null && (faction2 = ((TESEntityNPC) EntityList.createEntityByName(TESEntityRegistry.getStringFromClass(spawnClass2), worldObj)).getFaction()) != null && faction2.isBadRelation(spawnFaction);
	}
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote && ticksExisted % spawnInterval == 0 && (spawnClass1 != null || spawnClass2 != null) && worldObj.checkChunksExist(minX = (i = MathHelper.floor_double(posX)) - checkHorizontalRange, minY = (j = MathHelper.floor_double(boundingBox.minY)) + checkVerticalMin, minZ = (k = MathHelper.floor_double(posZ)) - checkHorizontalRange, maxX = i + checkHorizontalRange, maxY = j + checkVerticalMax, maxZ = k + checkHorizontalRange) && worldObj.getClosestPlayer(i + 0.5, j + 0.5, k + 0.5, noPlayerRange) == null && (entities = worldObj.selectEntitiesWithinAABB(EntityLiving.class, AxisAlignedBB.getBoundingBox(minX, minY, minZ, maxX + 1, maxY + 1, maxZ + 1), new EntitySelectorImpl(this)).size()) < spawnCap) {
			...
			for (int l = 0; l < attempts; ++l) {
				...
				TESEntityNPC entity = (TESEntityNPC) EntityList.createEntityByName(entityName, worldObj);
	...
	public void readSpawnerDataFromNBT(NBTTagCompound nbt) {
		...
		spawnClass1 = TESEntityRegistry.getClassFromString(nbt.getString("SpawnClass1"));
		spawnClass2 = TESEntityRegistry.getClassFromString(nbt.getString("SpawnClass2"));
		if (spawnClass1 != null && !TESEntityNPC.class.isAssignableFrom(spawnClass1)) {
			spawnClass1 = null;
		}
		if (spawnClass2 != null && !TESEntityNPC.class.isAssignableFrom(spawnClass2)) {
			spawnClass2 = null;
		}
	...
	public void writeSpawnerDataToNBT(NBTTagCompound nbt) {
		...
		String class2String = "";
		if (spawnClass1 != null) {
			class1String = TESEntityRegistry.getStringFromClass(spawnClass1);
		}
		if (spawnClass2 != null) {
			class2String = TESEntityRegistry.getStringFromClass(spawnClass2);
		}
	...
	private static class EntitySelectorImpl implements IEntitySelector {
		private final TESEntityNPCRespawner respawner;

		private EntitySelectorImpl(TESEntityNPCRespawner respawner) {
