...
public class TESEntityArrowFire extends EntityArrow implements IEntityAdditionalSpawnData {
	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityArrowFire(World world) {
		super(world);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityArrowFire(World world, double d, double d1, double d2) {
		super(world, d, d1, d2);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityArrowFire(World world, EntityLivingBase shooter, EntityLivingBase target, float charge, float inaccuracy) {
		super(world, shooter, target, charge, inaccuracy);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityArrowFire(World world, EntityLivingBase shooter, float charge) {
	...
	public void onCollideWithPlayer(EntityPlayer entityplayer) {
		...
		if (!worldObj.isRemote && isInGround && arrowShake <= 0) {
			...
			if (canBePickedUp == 1 && !entityplayer.inventory.addItemStackToInventory(new ItemStack(TESItems.arrowFire, 1))) {
				pickup = false;
			}
			if (pickup) {
