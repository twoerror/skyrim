...
import tes.common.entity.other.utils.TESFishing;
...
public class TESEntityFishHook extends EntityFishHook {
	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityFishHook(World world) {
		super(world);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityFishHook(World world, EntityPlayer entityplayer) {
	...
	public int func_146034_e() {
		...
		if (field_146043_c != null) {
			...
			damage = 3;
		} else if (TESReflection.getFishHookBobTime(this) > 0) {
			...
			TESFishing.FishResult result = TESFishing.getFishResult(rand, chance, luck, speed, true);
			...
			if (item.getItem() instanceof TESItemRing) {
				TESLevelData.getData(field_146042_b).addAchievement(TESAchievement.fishRing);
			}
			damage = 1;
		}
		if (TESReflection.isFishHookInGround(this)) {
			damage = 2;
		}
		setDead();
