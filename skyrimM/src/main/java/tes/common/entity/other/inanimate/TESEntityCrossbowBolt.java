...
public class TESEntityCrossbowBolt extends TESEntityProjectileBase {
	private double boltDamageFactor = 2.0;

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityCrossbowBolt(World world) {
		super(world);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityCrossbowBolt(World world, EntityLivingBase entityliving, EntityLivingBase target, ItemStack item, float charge, float inaccuracy) {
		super(world, entityliving, target, item, charge, inaccuracy);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityCrossbowBolt(World world, EntityLivingBase entityliving, ItemStack item, float charge) {
		super(world, entityliving, item, charge);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityCrossbowBolt(World world, ItemStack item, double d, double d1, double d2) {
	...
	public void onCollideWithTarget(Entity entity) {
		Item item;
		ItemStack itemstack;
		if (!worldObj.isRemote && entity instanceof EntityLivingBase && (itemstack = getProjectileItem()) != null && (item = itemstack.getItem()) instanceof TESItemCrossbowBolt && ((TESItemCrossbowBolt) item).isPoisoned()) {
