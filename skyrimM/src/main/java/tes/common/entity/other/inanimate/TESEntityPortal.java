...
public class TESEntityPortal extends Entity {
	...
	public TESEntityPortal(World world) {
		super(world);
		setSize(3.0f, 1.5f);
	}
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote && dimension != 0 && dimension != TESDimension.GAME_OF_THRONES.getDimensionID()) {
			setDead();
		}
	...
	private void transferEntity(Entity entity) {
		if (!worldObj.isRemote) {
			...
			TES.transferEntityToDimension(entity, dimension, new TESTeleporter(DimensionManager.getWorld(dimension), true));
		}
	}
