...
public class TESEntityBarrel extends Entity {
	...
	public TESEntityBarrel(World world) {
		super(world);
	...
	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityBarrel(World world, double d, double d1, double d2) {
		this(world);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (!worldObj.isRemote && !isDead) {
			...
			Block.SoundType stepSound = TESBlocks.barrel.stepSound;
	...
	public void entityInit() {
		...
		dataWatcher.addObject(20, new ItemStack(TESBlocks.barrel));
	}

	private ItemStack getBarrelDrop() {
		ItemStack itemstack = new ItemStack(TESBlocks.barrel);
		if (barrelItemData != null) {
			TESItemBarrel.setBarrelData(itemstack, barrelItemData);
		}
		return itemstack;
	}
			...
			if (!worldObj.isRemote) {
				...
				if (nearbyEntities != null && !nearbyEntities.isEmpty()) {
					for (Entity element : nearbyEntities) {
						if (element == riddenByEntity || !element.canBePushed() || !(element instanceof TESEntityBarrel)) {
							continue;
						}
