...
public class TESEntityMysteryWeb extends EntityThrowable {
	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityMysteryWeb(World world) {
		super(world);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityMysteryWeb(World world, double d, double d1, double d2) {
		super(world, d, d1, d2);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityMysteryWeb(World world, EntityLivingBase entityliving) {
	...
	public void onImpact(MovingObjectPosition m) {
		...
		if (!worldObj.isRemote) {
			...
			if (rand.nextInt(4) == 0) {
				TESEntityGiantSpider spider = new TESEntityGiantSpider(worldObj);
