...
public class TESEntityPebble extends EntityThrowable {
	private boolean isSling;

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityPebble(World world) {
		super(world);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityPebble(World world, double d, double d1, double d2) {
		super(world, d, d1, d2);
	}

	@SuppressWarnings({"WeakerAccess", "unused"})
	public TESEntityPebble(World world, EntityLivingBase entityliving) {
	...
	public void onImpact(MovingObjectPosition m) {
		...
		if (!worldObj.isRemote) {
			entityDropItem(new ItemStack(TESItems.pebble), 0.0f);
			setDead();
		}
	}
	...
	public TESEntityPebble setSling() {
		isSling = true;
		return this;
	}
