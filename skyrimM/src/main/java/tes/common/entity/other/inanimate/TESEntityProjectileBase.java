...
public abstract class TESEntityProjectileBase extends Entity implements IThrowableEntity, IProjectile {
	...
	protected TESEntityProjectileBase(World world) {
		super(world);
		setSize(0.5f, 0.5f);
	}

	protected TESEntityProjectileBase(World world, EntityLivingBase entityliving, EntityLivingBase target, ItemStack item, float charge, float inaccuracy) {
		super(world);
	...
	protected TESEntityProjectileBase(World world, EntityLivingBase entityliving, ItemStack item, float charge) {
		super(world);
		setProjectileItem(item);
		shootingEntity = entityliving;
		if (entityliving instanceof EntityPlayer) {
	...
	protected TESEntityProjectileBase(World world, ItemStack item, double d, double d1, double d2) {
		super(world);
		setProjectileItem(item);
		setSize(0.5f, 0.5f);
		setPosition(d, d1, d2);
		...
		if (inGround) {
			...
			if (movingobjectposition != null) {
				...
				if (hitEntity != null) {
					...
					if (itemstack != null) {
						knockbackStrength = knockbackStrength + (shootingEntity instanceof EntityLivingBase && hitEntity instanceof EntityLivingBase ? EnchantmentHelper.getKnockbackModifier((EntityLivingBase) shootingEntity, (EntityLivingBase) hitEntity) : TESWeaponStats.getTotalKnockback(itemstack));
					}
					if (getIsCritical()) {
