...
public class TESEntitySnowBear extends EntityAnimal implements TESBiome.ImmuneToFrost {
	...
	public TESEntitySnowBear(World world) {
		super(world);
		setSize(2.1f, 2.35f);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (flag && (attacker = damagesource.getEntity()) instanceof EntityLivingBase) {
			if (isChild()) {
				...
				for (Entity obj : list) {
					if (obj instanceof TESEntitySnowBear && !((EntityLivingBase) obj).isChild()) {
						((TESEntitySnowBear) obj).becomeAngryAt((EntityLivingBase) attacker);
					}
				}
			} else {
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new TESEntitySnowBear(worldObj);
	}
