...
public class TESEntitySwan extends EntityCreature implements TESAmbientCreature, TESBiome.ImmuneToFrost {
	private static final Random VIOLENCE_RAND = new Random();

	private final EntityAIBase attackAI = new TESEntityAIAttackOnCollide(this, 1.4, true);
	...
	public TESEntitySwan(World world) {
		super(world);
		setSize(0.5f, 0.7f);
	...
	public void dropFewItems(boolean flag, int i) {
		...
		for (int l = 0; l < feathers; ++l) {
			dropItem(TESItems.swanFeather, 1);
		}
	}
	...
	public float getBlockPathWeight(int i, int j, int k) {
		return worldObj.getBlock(i, j - 1, k) == TESCrashHandler.getBiomeGenForCoords(worldObj, i, k).topBlock ? 10.0f : worldObj.getLightBrightness(i, j, k) - 0.5f;
	}
	...
	public void updateAITasks() {
		...
		if (timeUntilHiss <= 0) {
			...
			double range;
			if (getAttackTarget() == null && rand.nextInt(3) == 0 && !(nearbyPlayers = worldObj.selectEntitiesWithinAABB(EntityPlayer.class, boundingBox.expand(range = 8.0, range, range), TES.selectNonCreativePlayers())).isEmpty()) {
				...
				playSound("tes:swan.hiss", getSoundVolume(), getSoundPitch());
	...
	private static class EntitySelectorImpl implements IEntitySelector {
		private final TESEntitySwan swan;

		private EntitySelectorImpl(TESEntitySwan swan) {
			this.swan = swan;
		}
