...
public class TESEntityWalrus extends EntityAnimal implements TESBiome.ImmuneToFrost {
	...
	public TESEntityWalrus(World world) {
		super(world);
		setSize(1.05f, 1.35f);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (flag && (attacker = damagesource.getEntity()) instanceof EntityLivingBase) {
			if (isChild()) {
				...
				for (Entity obj : list) {
					if (obj instanceof TESEntityWalrus && !((EntityLivingBase) obj).isChild()) {
						((TESEntityWalrus) obj).becomeAngryAt((EntityLivingBase) attacker);
					}
				}
			} else {
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new TESEntityWalrus(worldObj);
	}
	...
	public void dropFewItems(boolean flag, int i) {
		...
		for (int l = 0; l < meat; ++l) {
			if (isBurning()) {
				dropItem(TESItems.walrusLardCooked, 1);
				continue;
			}
			dropItem(TESItems.walrusLardRaw, 1);
		}
	}
