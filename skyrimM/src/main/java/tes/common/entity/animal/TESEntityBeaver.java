...
public class TESEntityBeaver extends EntityAnimal implements TESBiome.ImmuneToFrost {
	...
	public TESEntityBeaver(World world) {
		super(world);
		setSize(1.05f, 1.35f);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (flag && (attacker = damagesource.getEntity()) instanceof EntityLivingBase) {
			if (isChild()) {
				...
				for (Entity obj : list) {
					if (obj instanceof TESEntityBeaver && !((EntityLivingBase) obj).isChild()) {
						((TESEntityBeaver) obj).becomeAngryAt((EntityLivingBase) attacker);
					}
				}
			} else {
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new TESEntityBeaver(worldObj);
	}

	@Override
	public void dropFewItems(boolean flag, int i) {
		if (isBurning()) {
			dropItem(TESItems.beaverCooked, 1);
		} else {
			dropItem(TESItems.beaverRaw, 1);
		}
		dropItem(TESItems.beaverTail, 1);
	}
	...
	public void onDeath(DamageSource damagesource) {
		...
		if (!worldObj.isRemote && damagesource.getEntity() instanceof EntityPlayer) {
			...
			TESLevelData.getData(entityplayer).addAchievement(TESAchievement.killBeaver);
		}
	}
