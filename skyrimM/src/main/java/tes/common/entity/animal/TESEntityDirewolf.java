...
public class TESEntityDirewolf extends EntityAnimal implements TESBiome.ImmuneToFrost {
	...
	public TESEntityDirewolf(World world) {
		super(world);
		setSize(0.9F, 1.1F);
	...
	public boolean attackEntityFrom(DamageSource damagesource, float f) {
		...
		if (flag && (attacker = damagesource.getEntity()) instanceof EntityLivingBase) {
			if (isChild()) {
				...
				for (Entity obj : list) {
					if (obj instanceof TESEntityDirewolf && !((EntityLivingBase) obj).isChild()) {
						((TESEntityDirewolf) obj).becomeAngryAt((EntityLivingBase) attacker);
					}
				}
			} else {
	...
	public EntityAgeable createChild(EntityAgeable entity) {
		return new TESEntityDirewolf(worldObj);
	}
