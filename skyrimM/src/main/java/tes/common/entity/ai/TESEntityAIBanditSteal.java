...
public class TESEntityAIBanditSteal extends EntityAIBase {
	...
	public TESEntityAIBanditSteal(TESEntityBanditBase bandit, double d) {
		theBandit = bandit;
	...
	public boolean continueExecuting() {
		return targetPlayer != null && targetPlayer.isEntityAlive() && !targetPlayer.capabilities.isCreativeMode && TESEntityBanditBase.canStealFromPlayerInv(targetPlayer) && chaseTimer > 0 && theBanditAsNPC.getDistanceSqToEntity(targetPlayer) < 256.0;
	}
	...
	public boolean shouldExecute() {
		...
		for (EntityPlayer player : players) {
			if (player.capabilities.isCreativeMode || !TESEntityBanditBase.canStealFromPlayerInv(player)) {
				continue;
			}
	...
	private void steal() {
		...
		for (int i = 0; i < thefts; ++i) {
			if (tryStealItem(inv, TESItemCoin.class)) {
				stolenSomething = true;
			}
			if (tryStealItem(inv, TESItemGem.class) || tryStealItem(inv, TESItemRing.class) || tryStealItem(inv, ItemArmor.class)) {
				stolenSomething = true;
				continue;
			}
			if (tryStealItem(inv, ItemSword.class) || tryStealItem(inv, ItemTool.class) || tryStealItem(inv, TESItemPouch.class)) {
				stolenSomething = true;
				continue;
			}
			if (!tryStealItem(inv)) {
		...
		if (stolenSomething) {
			targetPlayer.addChatMessage(TESEntityBanditBase.getTheftChatMsg());
			...
			TESLevelData.getData(targetPlayer).cancelFastTravel();
		}
	}
