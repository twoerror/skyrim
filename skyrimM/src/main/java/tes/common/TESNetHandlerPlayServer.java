...
public class TESNetHandlerPlayServer extends NetHandlerPlayServer {
	...
	public TESNetHandlerPlayServer(MinecraftServer server, NetworkManager nm, EntityPlayerMP entityplayer) {
	...
	public void processInput(C0CPacketInput packet) {
		...
		if (forward != 0.0f || strafing != 0.0f || jump) {
			TESLevelData.getData(playerEntity).cancelFastTravel();
		}
	}

	public void processMountControl(TESPacketMountControl packet) {
		...
		if (mount != null && mount != playerEntity && mount.riddenByEntity == playerEntity && TESMountFunctions.isMountControllable(mount)) {
			...
			if (distSq - speedSq > 150.0 && (!server.isSinglePlayer() || !server.getServerOwner().equals(playerEntity.getCommandSenderName()))) {
				...
				IMessage pktClient = new TESPacketMountControlServerEnforce(mount);
				TESPacketHandler.NETWORK_WRAPPER.sendTo(pktClient, playerEntity);
				return;
			}
			double check = 0.0625;
	...
	public void processUseEntity(C02PacketUseEntity packet) {
		...
		if (target != null) {
			...
			double reach = TESWeaponStats.getMeleeReachDistance(playerEntity);
			reach += 1.0f;
			reach += target.getCollisionBorderSize();
			int attackTime = TESWeaponStats.getAttackTimePlayer(itemstack);
	...
	private void setBlockReach() {
		...
		playerEntity.theItemInWorldManager.setBlockReachDistance(reach * TESWeaponStats.getMeleeReachFactor(playerEntity.getHeldItem()));
	}
