...
public class TESModelCargocart extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float rotationYaw, float rotationPitch, float scale, Entity entity) {
		...
		leftWheel.rotateAngleX = ((TESEntityCart) entity).getWheelRotation();
		rightWheel.rotateAngleX = ((TESEntityCart) entity).getWheelRotation();
		leftWheel.render(scale);
		rightWheel.render(scale);
		if (((TESEntityCargocart) entity).getLoad() > 3) {
			cargo.render(scale);
			if (((TESEntityCargocart) entity).getLoad() > 8) {
				cargo1.render(scale);
				if (((TESEntityCargocart) entity).getLoad() > 16) {
					cargo2.render(scale);
					if (((TESEntityCargocart) entity).getLoad() > 31) {
						cargo3.render(scale);
					}
				}
			}
