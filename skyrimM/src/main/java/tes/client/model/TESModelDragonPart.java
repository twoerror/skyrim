...
public class TESModelDragonPart extends ModelRenderer {
	...
	public TESModelDragonPart(ModelBase base, String name) {
		super(base, name);
		this.base = base;
	}

	public TESModelDragonPart addChildBox(String name, float xOfs, float yOfs, float zOfs, int width, int length, int height) {
		TESModelDragonPart part = new TESModelDragonPart(base, boxName);
		part.mirror = mirror;
	...
	public void postRender(float scale) {
		...
		if (preRotateAngleZ != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(preRotateAngleZ), 0, 0, 1);
		}
		if (preRotateAngleY != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(preRotateAngleY), 0, 1, 0);
		}
		if (preRotateAngleX != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(preRotateAngleX), 1, 0, 0);
		}
		if (rotateAngleZ != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(rotateAngleZ), 0, 0, 1);
		}
		if (rotateAngleY != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(rotateAngleY), 0, 1, 0);
		}
		if (rotateAngleX != 0) {
			glRotatef(TESModelDragonAnimaton.toDegrees(rotateAngleX), 1, 0, 0);
		}
	...
	public TESModelDragonPart setAngles(float x, float y, float z) {
		rotateAngleX = x;
		rotateAngleY = y;
		rotateAngleZ = z;
		return this;
