...
import tes.common.util.TESCircularBuffer;
import tes.common.util.TESTickFloat;
import net.minecraft.util.MathHelper;

public class TESModelDragonAnimaton {
	...
	private final TESEntityDragon entity;
	private final TESTickFloat animTimer = new TESTickFloat();
	private final TESTickFloat groundTimer = new TESTickFloat(1).setLimit(0, 1);
	private final TESTickFloat flutterTimer = new TESTickFloat().setLimit(0, 1);
	private final TESTickFloat walkTimer = new TESTickFloat().setLimit(0, 1);
	private final TESTickFloat sitTimer = new TESTickFloat().setLimit(0, 1);
	private final TESTickFloat jawTimer = new TESTickFloat().setLimit(0, 1);
	private final TESTickFloat speedTimer = new TESTickFloat(1).setLimit(0, 1);
	private final TESCircularBuffer yTrail = new TESCircularBuffer(8);
	private final TESCircularBuffer yawTrail = new TESCircularBuffer(16);
	private final TESCircularBuffer pitchTrail = new TESCircularBuffer(16);
	...
	public TESModelDragonAnimaton(TESEntityDragon dragon) {
		entity = dragon;
	}
	...
	private static void interp(float x, float[] result, float[]... knots) {
		...
		int knot = 0;
		if (nspans < 1) {
			System.out.println(TESModelDragonAnimaton.class.getName() + " Spline has too few knots");
			return;
		}
	...
	public void animate(TESModelDragon model) {
	...
	private void animHeadAndNeck(TESModelDragon model) {
		...
		for (int i = 0; i < model.getNeckProxyLength(); i++) {
			...
			neckSize = TESModelDragon.NECK_SIZE * model.getNeck().getRenderScaleZ() - 1.4f;
	...
	private void animLegs(TESModelDragon model) {
		if (ground < 1) {
		...
		for (int i = 0; i < model.getThighProxyLength(); i++) {
			TESModelDragonPart thigh;
			TESModelDragonPart crus;
			TESModelDragonPart foot;
			TESModelDragonPart toe;
			if (i % 2 == 0) {
	...
	private void animTail(TESModelDragon model) {
		...
		for (int i = 0; i < model.getTailProxyLength(); i++) {
			...
			float tailSize = TESModelDragon.TAIL_SIZE * model.getTail().getRenderScaleZ() - 0.7f;
	...
	private void animWings(TESModelDragon model) {
