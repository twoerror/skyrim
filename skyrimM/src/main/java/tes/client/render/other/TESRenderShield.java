...
public class TESRenderShield {
	private static final float MODELSCALE = 0.0625f;

	private TESRenderShield() {
	}
	...
	public static void renderShield(TESShields shield, EntityLivingBase entity, ModelBiped model) {
		...
		boolean wearingChestplate = chestplate != null && chestplate.getItem().isValidArmor(chestplate, ((ItemArmor) TESItems.bronzeChestplate).armorType, entity);
		boolean renderOnBack = (!holdingSword || heldLeft != null) && entity != null;
		boolean doNotRender = entity instanceof TESEntityNPC && ((TESEntityNPC) entity).getCape() != null && renderOnBack || entity instanceof EntityPlayer && TESLevelData.getData((EntityPlayer) entity).getCape() != null && renderOnBack;
		if (!doNotRender) {
			GL11.glPushMatrix();
			if (renderOnBack) {
