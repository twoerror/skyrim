...
import tes.common.item.weapon.TESItemPike;
...
public class TESRenderLargeItem implements IItemRenderer {
	...
	private final Collection<TESExtraLargeIconToken> extraTokens = new ArrayList<>();
	...
	private TESRenderLargeItem(Item item, String dir, float f) {
		theItem = item;
		folderName = dir;
		largeIconScale = f;
	}
	...
	public static TESRenderLargeItem getRendererIfLarge(Item item) {
		for (Map.Entry<String, Float> entry : SIZE_FOLDERS.entrySet()) {
			...
			try {
				...
				if (res != null) {
					return new TESRenderLargeItem(item, folder, iconScale);
				}
	...
	public TESExtraLargeIconToken extraIcon(String name) {
		TESExtraLargeIconToken token = new TESExtraLargeIconToken(name);
	...
	public void registerIcons(IIconRegister register) {
		...
		for (TESExtraLargeIconToken token : extraTokens) {
	...
	public void renderItem(IItemRenderer.ItemRenderType type, ItemStack itemstack, Object... data) {
		...
		if (item instanceof TESItemSpear && holder instanceof EntityPlayer && ((EntityPlayer) holder).getItemInUse() == itemstack) {
			GL11.glRotatef(260.0f, 0.0f, 0.0f, 1.0f);
			GL11.glTranslatef(-1.0f, 0.0f, 0.0f);
		}
		if (item instanceof TESItemPike && holder instanceof EntityLivingBase && (entityliving = (EntityLivingBase) holder).getHeldItem() == itemstack && entityliving.swingProgress <= 0.0f) {
		...
		if (item instanceof TESItemLance && holder instanceof EntityLivingBase && ((EntityLivingBase) holder).getHeldItem() == itemstack) {
			if (isFirstPerson) {
				GL11.glRotatef(260.0f, 0.0f, 0.0f, 1.0f);
			} else {
	...
	public void renderLargeItem(TESExtraLargeIconToken token) {
