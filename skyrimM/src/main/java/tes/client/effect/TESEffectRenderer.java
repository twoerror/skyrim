...
public class TESEffectRenderer {
	public static final TESEffectRenderer INSTANCE = new TESEffectRenderer();

	private List<EntityFX>[] particleLayers = new List[0];

	private TESEffectRenderer() {
	}
	...
	public void renderParticles(Entity entity, float f) {
		...
		for (int l = 0; l < particleLayers.length; ++l) {
			...
			if (!layer.isEmpty()) {
				if (l == 0) {
					mc.getTextureManager().bindTexture(TESClientProxy.PARTICLES_TEXTURE);
				} else if (l == 1) {
					mc.getTextureManager().bindTexture(TESClientProxy.PARTICLES_2_TEXTURE);
				}
