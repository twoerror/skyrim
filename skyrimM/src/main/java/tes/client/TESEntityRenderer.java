...
public class TESEntityRenderer extends EntityRenderer {
	private final Minecraft theMC;

	public TESEntityRenderer(Minecraft mc, IResourceManager irm) {
		super(mc, irm);
		theMC = mc;
	}
	...
	public void getMouseOver(float partialTick) {
		if (theMC.renderViewEntity != null && theMC.theWorld != null) {
			...
			float meleeReachFactor = TESWeaponStats.getMeleeReachFactor(theMC.thePlayer.getHeldItem());
			theMC.objectMouseOver = theMC.renderViewEntity.rayTrace(blockReach * meleeReachFactor, partialTick);
			double maxDist = reach = TESWeaponStats.getMeleeReachDistance(theMC.thePlayer);
	...
	public void updateRenderer() {
		...
		if (Minecraft.isGuiEnabled()) {
			float wight = TESClientFactory.getTickHandlerClient().getWightLookFactor();
			float hand = TESReflectionClient.getHandFOV(this);
			TESReflectionClient.setHandFOV(this, hand + wight * 0.3f);
		}
	}
}