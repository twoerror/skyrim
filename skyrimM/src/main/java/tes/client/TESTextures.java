...
import tes.client.render.other.TESBufferedImageIcon;
import tes.common.TESConfig;
import tes.common.TESDimension;
import tes.common.util.TESColorUtil;
...
public class TESTextures implements IResourceManagerReloadListener {
	public static final ResourceLocation OVERLAY_TEXTURE = new ResourceLocation("tes:textures/map/mapOverlay.png");
	public static final ResourceLocation OSRS_TEXTURE = new ResourceLocation("tes:textures/map/osrs.png");

	public static final TESTextures INSTANCE = new TESTextures();

	private static final Minecraft MC = Minecraft.getMinecraft();

	public static final ResourceLocation MISSING_TEXTURE = MC.getTextureManager().getDynamicTextureLocation("tes.missingSkin", TextureUtil.missingTexture);

	private static final ResourceLocation PARTICLE_TEXTURES = new ResourceLocation("textures/particle/particles.png");
	private static final ResourceLocation NEW_WATER_PARTICLES = new ResourceLocation("tes:textures/misc/waterParticles.png");
				...
				for (int j = 0; j < mapHeight; ++j) {
					...
					if (rgb == 5468426) {
						...
						if (water > 0) {
							...
							rgb = TESColorUtil.lerpColorsI(5468426, 9279778, ratio * 2.0f);
						}
						...
						if (edge > 0) {
							...
							rgb = TESColorUtil.lerpColorsI(14736861, 9005125, ratio * 1.5f);
						}
					}
	...
	public static void drawMap(boolean sepia, double x0, double x1, double y0, double y1, double z, double minU, double maxU, double minV, double maxV, float alpha) {
		...
		int mtY = 0;
		int mtW = 20;
		double mtMinU = (double) (mtX - mtW) / TESGenLayerWorld.getImageWidth();
		double mtMaxU = (double) (mtX + mtW) / TESGenLayerWorld.getImageWidth();
		double mtMinV = (double) (mtY - mtW) / TESGenLayerWorld.getImageHeight();
		double mtMaxV = (double) (mtY + mtW) / TESGenLayerWorld.getImageHeight();
	...
	public static void drawMapCompassBottomLeft(double x, double y, double z, double scale) {
		...
		MC.getTextureManager().bindTexture(TESGuiMap.MAP_ICONS_TEXTURE);
		int width = 32;
		int height = 32;
	...
	private static IIcon generateIconEmpty(TextureMap textureMap) {
		String iconName = "textures/blocks/TES_EMPTY_ICON";
		int size = 16;
	...
	public static ResourceLocation getEyesTexture(ResourceLocation skin, int[][] eyesCoords, int eyeWidth, int eyeHeight) {
		...
		if (eyes == null) {
			try {
				...
				TESLog.getLogger().error("Failed to generate eyes skin");
				e.printStackTrace();
	...
	public static int getMapOceanColor(boolean sepia) {
		...
		int ocean = TESBiome.ocean.color;
		if (sepia) {
	...
	private static void loadMapTextures() {
		mapTexture = new ResourceLocation("tes:textures/map/map.png");
		try {
			BufferedImage mapImage = ImageIO.read(MC.getResourceManager().getResource(mapTexture).getInputStream());
			sepiaMapTexture = convertToSepia(mapImage, new ResourceLocation("tes:textures/map_sepia"));
		} catch (IOException e) {
	...
	public static void onInit() {
		...
		TESTextures textures = TESClientFactory.getTextures();
	...
	public static void setMapTexture(ResourceLocation mapTexture) {
		TESTextures.mapTexture = mapTexture;
	}

	public static void setSepiaMapTexture(ResourceLocation sepiaMapTexture) {
		TESTextures.sepiaMapTexture = sepiaMapTexture;
	}
	...
	public void preTextureStitch(TextureStitchEvent.Pre event) {
		...
		if (map.getTextureType() == 0) {
			TESCommonIcons.iconEmptyBlock = generateIconEmpty(map);
			TESCommonIcons.iconStoneSnow = map.registerIcon("stone_snow");
		}
		if (map.getTextureType() == 1) {
			TESCommonIcons.iconMeleeWeapon = map.registerIcon("tes:slot_melee");
			TESCommonIcons.iconBomb = map.registerIcon("tes:slot_bomb");
		}
	}
}