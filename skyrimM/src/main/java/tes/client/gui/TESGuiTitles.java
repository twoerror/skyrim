...
import tes.client.TESReflectionClient;
import tes.common.TESLevelData;
import tes.common.database.TESTitle;
import tes.common.network.TESPacketHandler;
import tes.common.network.TESPacketSelectTitle;
...
public class TESGuiTitles extends TESGuiMenuBase {
	...
	private final List<TESTitle> displayedTitles = new ArrayList<>();
	private final Map<TESTitle, Pair<Boolean, Pair<Integer, Integer>>> displayedTitleInfo = new HashMap<>();
	...
	private TESTitle.PlayerTitle currentTitle;
	private TESTitle selectedTitle;
	...
	public void actionPerformed(GuiButton button) {
		if (button.enabled) {
			if (button == selectButton && (currentTitle == null || selectedTitle != currentTitle.getTitle() || selectedColor != currentTitle.getColor())) {
				IMessage packet = new TESPacketSelectTitle(new TESTitle.PlayerTitle(selectedTitle, selectedColor));
				TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
			} else if (button == removeButton) {
				IMessage packet = new TESPacketSelectTitle(null);
				TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
	...
	public void drawScreen(int i, int j, float f) {
		...
		setupScrollBar(i, j);
		String titleName = currentTitle == null ? StatCollector.translateToLocal("tes.gui.titles.currentTitle.none") : currentTitle.getTitle().getDisplayName(mc.thePlayer);
		...
		drawCenteredString(StatCollector.translateToLocalFormatted("tes.gui.titles.currentTitle", titleName), guiLeft + sizeX / 2, guiTop, 16777215);
		...
		for (int index = min; index <= max; ++index) {
			String name;
			TESTitle title = displayedTitles.get(index);
		...
		if (selectedTitle != null) {
			...
			for (EnumChatFormatting code : colorCodes) {
				int color = TESReflectionClient.getFormattingColor(code);
		...
		for (Map.Entry<TESTitle, Pair<Boolean, Pair<Integer, Integer>>> entry : displayedTitleInfo.entrySet()) {
			TESTitle title = entry.getKey();
	...
	public void initGui() {
		...
		selectButton = new TESGuiButton(0, guiLeft + sizeX / 2 - 290, guiTop + 90, 80, 20, StatCollector.translateToLocal("tes.gui.titles.select"));
		buttonList.add(selectButton);
		removeButton = new TESGuiButton(1, guiLeft + sizeX / 2 - 200, guiTop + 90, 80, 20, StatCollector.translateToLocal("tes.gui.titles.remove"));
		buttonList.add(removeButton);
		goBack = new TESGuiButton(2, guiLeft + sizeX / 2 - 290, guiTop + 120, 170, 20, StatCollector.translateToLocal("tes.gui.menuButton"));
	...
	public void updateScreen() {
		super.updateScreen();
		currentTitle = TESLevelData.getData(mc.thePlayer).getPlayerTitle();
		displayedTitles.clear();
		List<TESTitle> availableTitles = new ArrayList<>();
		List<TESTitle> unavailableTitles = new ArrayList<>();
		for (TESTitle title : TESTitle.CONTENT) {
		...
		Comparator<TESTitle> sorter = TESTitle.createTitleSorter(mc.thePlayer);
