...
public class TESGuiMiniquestOffer extends TESGuiScreenBase {
	private static final ResourceLocation GUI_TEXTURE = new ResourceLocation("tes:textures/gui/quest/miniquest.png");
	private static final RenderItem ITEM_RENDERER = new RenderItem();

	private static final int X_SIZE = 256;
	private static final int Y_SIZE = 200;

	private final TESMiniQuest theMiniQuest;
	...
	public TESGuiMiniquestOffer(TESMiniQuest quest, TESEntityNPC npc) {
		theMiniQuest = quest;
		theNPC = npc;
		rand = theNPC.getRNG();
		openTick = 0;
	...
	public void actionPerformed(GuiButton button) {
		if (button.enabled) {
			...
			if (button == buttonAccept) {
				TESPacketMiniquestOffer.sendClosePacket(mc.thePlayer, theNPC, true);
				close = true;
			} else if (button == buttonDecline) {
				TESPacketMiniquestOffer.sendClosePacket(mc.thePlayer, theNPC, false);
				close = true;
			}
			if (close) {
	...
	public void drawScreen(int i, int j, float f) {
		if (description == null) {
			description = TESSpeech.formatSpeech(theMiniQuest.getQuoteStart(), mc.thePlayer, null, theMiniQuest.getObjectiveInSpeech());
		}
	...
	public void initGui() {
		...
		buttonAccept = new TESGuiButton(0, guiLeft + X_SIZE / 2 - 20 - 80, guiTop + Y_SIZE - 30, 80, 20, StatCollector.translateToLocal("tes.gui.miniquestOffer.accept"));
		buttonList.add(buttonAccept);
		buttonDecline = new TESGuiButton(1, guiLeft + X_SIZE / 2 + 20, guiTop + Y_SIZE - 30, 80, 20, StatCollector.translateToLocal("tes.gui.miniquestOffer.decline"));
	...
	private void renderNPC(int i, int j, float dx, float dy, float f) {
		...
		if (render instanceof TESRenderBiped) {
			TESRenderBiped npcRenderer = (TESRenderBiped) render;
