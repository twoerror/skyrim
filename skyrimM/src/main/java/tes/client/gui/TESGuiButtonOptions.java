...
public class TESGuiButtonOptions extends GuiButton {
	private final String baseDisplayString;

	public TESGuiButtonOptions(int i, int j, int k, int l, int i1, String s) {
		super(i, j, k, l, i1, s);
		baseDisplayString = s;
	}
	...
	public void drawButton(Minecraft mc, int i, int j) {
		if (visible) {
			...
			mc.getTextureManager().bindTexture(TESGuiQuestBook.GUI_TEXTURE);
	...
	public void setState(boolean flag) {
		setState(flag ? StatCollector.translateToLocal("tes.gui.button.on") : StatCollector.translateToLocal("tes.gui.button.off"));
	}
