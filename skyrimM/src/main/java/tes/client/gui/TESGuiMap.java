...
import tes.common.faction.TESControlZone;
...
public class TESGuiMap extends TESGuiMenuBaseReturn {
	public static final ResourceLocation MAP_ICONS_TEXTURE = new ResourceLocation("tes:textures/map/mapScreen.png");

	private static final Map<TESDimension.DimensionRegion, TESFaction> LAST_VIEWED_REGIONS = new EnumMap<>(TESDimension.DimensionRegion.class);
	private static final Map<UUID, PlayerLocationInfo> PLAYER_LOCATIONS = new HashMap<>();
	private static final Collection<TESGuiMapWidget> MAP_WIDGETS = new ArrayList<>();

	private static final ResourceLocation CONQUEST_TEXTURE = new ResourceLocation("tes:textures/map/conquest.png");
	private static final ItemStack QUEST_BOOK_ICON = new ItemStack(TESItems.questBook);
	...
	private final Set<TESFaction> requestedFacGrids = EnumSet.noneOf(TESFaction.class);
	private final Set<TESFaction> receivedFacGrids = EnumSet.noneOf(TESFaction.class);
	private final Map<TESFaction, List<TESConquestZone>> facConquestGrids = new EnumMap<>(TESFaction.class);

	private final TESGuiScrollPane scrollPaneWPShares = new TESGuiScrollPane(9, 8);

	private TESGuiMapWidget widgetAddCWP;
	private TESGuiMapWidget widgetDelCWP;
	private TESGuiMapWidget widgetRenameCWP;
	private TESGuiMapWidget widgetToggleWPs;
	private TESGuiMapWidget widgetToggleCWPs;
	private TESGuiMapWidget widgetToggleHiddenSWPs;
	private TESGuiMapWidget widgetZoomIn;
	private TESGuiMapWidget widgetZoomOut;
	private TESGuiMapWidget widgetFullScreen;
	private TESGuiMapWidget widgetSepia;
	private TESGuiMapWidget widgetLabels;
	private TESGuiMapWidget widgetShareCWP;
	private TESGuiMapWidget widgetHideSWP;
	private TESGuiMapWidget widgetUnhideSWP;

	private List<UUID> displayedWPShareList;

	private TESFaction conquestViewingFaction;
	private TESFaction controlZoneFaction;

	private TESAbstractWaypoint selectedWaypoint;
	private TESFellowshipClient mouseOverRemoveSharedFellowship;
	private TESGuiFellowships fellowshipDrawGUI;
	...
	public static void setShowHiddenSWP(boolean showHiddenSWP) {
		TESGuiMap.showHiddenSWP = showHiddenSWP;
	}
	...
	private static boolean isWaypointVisible(TESAbstractWaypoint wp) {
		...
		if (button.enabled) {
			if (button == buttonOverlayFunction) {
				if (creatingWaypoint) {
					...
					IMessage packet = new TESPacketCreateCWP(waypointName);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					closeOverlay();
				} else if (deletingWaypoint) {
					IMessage packet = new TESPacketDeleteCWP(selectedWaypoint);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					closeOverlay();
					selectedWaypoint = null;
				} else if (renamingWaypoint && isValidWaypointName(nameWPTextField.getText())) {
					String newName = nameWPTextField.getText();
					IMessage packet = new TESPacketRenameCWP(selectedWaypoint, newName);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					closeOverlay();
				} else if (sharingWaypoint) {
					openOverlayShareNew();
				} else if (sharingWaypointNew && isExistingUnsharedFellowshipName(nameWPTextField.getText(), (TESCustomWaypoint) selectedWaypoint)) {
					String fsName = nameWPTextField.getText();
					IMessage packet = new TESPacketShareCWP(selectedWaypoint, fsName, true);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					openOverlayShare();
				}
			} else if (button == buttonConquestRegions) {
				List<TESDimension.DimensionRegion> regionList = TESDimension.GAME_OF_THRONES.getDimensionRegions();
	...
	private boolean canCreateWaypointAtPosition() {
		int minY = TESConfig.getCustomWaypointMinY(mc.theWorld);
	...
	public void drawScreen(int i, int j, float f) {
		...
		posX += posXMove * f;
		posY += posYMove * f;
		boolean isSepia = isConquestGrid || TESConfig.enableSepiaMap;
		if (isConquestGrid) {
			drawDefaultBackground();
		}
		if (fullscreen || isConquestGrid) {
			mc.getTextureManager().bindTexture(TESTextures.OVERLAY_TEXTURE);
		...
		setupScrollBars(i, j);
		GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		int oceanColor = TESTextures.getMapOceanColor(isSepia);
		drawRect(mapXMin, mapYMin, mapXMax, mapYMax, oceanColor);
		GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		if (!isConquestGrid) {
			String title = StatCollector.translateToLocal("tes.gui.map.title");
			if (fullscreen) {
		...
		if (hasControlZones && TESFaction.controlZonesEnabled(mc.theWorld)) {
			...
			String tooltip = null;
			if (mouseControlZone) {
				tooltip = StatCollector.translateToLocal("tes.gui.map.controlZoneFull");
			} else if (mouseControlZoneReduced) {
				tooltip = StatCollector.translateToLocal("tes.gui.map.controlZoneReduced");
			}
			if (tooltip != null) {
			...
			if (isConquestGrid && conquestViewingFaction != null) {
				...
				if (!loadingConquestGrid) {
					...
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
					List<TESConquestZone> allZones = facConquestGrids.get(conquestViewingFaction);
					if (allZones == null) {
						allZones = new ArrayList<>();
					}
					List<TESConquestZone> zonesInView = new ArrayList<>();
					highestViewedConqStr = 0.0f;
					float mouseOverStr = 0.0f;
					TESConquestZone mouseOverZone = null;
					TESConquestGrid.ConquestEffective mouseOverEffect = null;
					for (int pass = 0; pass <= 1; ++pass) {
						if (pass != 1 || highestViewedConqStr > 0.0f) {
							List<TESConquestZone> zoneList = pass == 0 ? allZones : zonesInView;
							for (TESConquestZone zone : zoneList) {
								float strength = zone.getConquestStrength(conquestViewingFaction, mc.theWorld);
								int[] min = TESConquestGrid.getMinCoordsOnMap(zone);
								int[] max = TESConquestGrid.getMaxCoordsOnMap(zone);
								...
								if (maxX >= mapXMin && minX <= mapXMax && maxY >= mapYMin && minY <= mapYMax) {
									...
									if (strength > 0.0f) {
										...
										TESConquestGrid.ConquestEffective effect = TESConquestGrid.getConquestEffectIn(mc.theWorld, zone, conquestViewingFaction);
										int zoneColor = 0xBB0000 | Math.round(strAlpha * 255.0f) << 24;
										if (effect == TESConquestGrid.ConquestEffective.EFFECTIVE) {
											drawFloatRect(minX, minY, maxX, maxY, zoneColor);
										} else {
											int zoneColor2 = 0x1E1E1E | Math.round(strAlpha * 255.0f) << 24;
											if (effect == TESConquestGrid.ConquestEffective.ALLY_BOOST) {
											...
											} else if (effect == TESConquestGrid.ConquestEffective.NO_EFFECT) {
					...
					if (mouseOverZone != null && i >= mapXMin && i < mapXMax && j >= mapYMin && j < mapYMax) {
						int[] min = TESConquestGrid.getMinCoordsOnMap(mouseOverZone);
						int[] max = TESConquestGrid.getMaxCoordsOnMap(mouseOverZone);
						float[] minF = transformMapCoords(min[0], min[1]);
						float[] maxF = transformMapCoords(max[0], max[1]);
						float minX = minF[0];
						float maxX = maxF[0];
						...
						String tooltip = TESAlignmentValues.formatConqForDisplay(mouseOverStr, false);
						String subtip = null;
						if (mouseOverEffect == TESConquestGrid.ConquestEffective.ALLY_BOOST) {
							subtip = StatCollector.translateToLocalFormatted("tes.gui.map.conquest.allyBoost", conquestViewingFaction.factionName());
						} else if (mouseOverEffect == TESConquestGrid.ConquestEffective.NO_EFFECT) {
							subtip = StatCollector.translateToLocal("tes.gui.map.conquest.noEffect");
						}
						int strWidth = mc.fontRenderer.getStringWidth(tooltip);
		...
		zLevel += 50.0f;
		TESTextures.drawMapCompassBottomLeft(mapXMin + 12, mapYMax - 12, zLevel, 1.0);
		zLevel -= 50.0f;
		renderBeziers();
		renderPlayers(i, j);
		if (!loadingConquestGrid) {
		...
		if (isConquestGrid) {
			if (loadingConquestGrid) {
				...
				TESConquestGrid.ConquestViewableQuery query = TESConquestGrid.canPlayerViewConquest(mc.thePlayer, conquestViewingFaction);
				if (query.getResult() == TESConquestGrid.ConquestViewable.CAN_VIEW) {
					loadText = new StringBuilder(StatCollector.translateToLocal("tes.gui.map.conquest.wait"));
					int ellipsis = 1 + tickCounter / 10 % 3;
					for (int l = 0; l < ellipsis; ++l) {
						loadText.append('.');
					}
				} else if (query.getResult() == TESConquestGrid.ConquestViewable.UNPLEDGED) {
					loadText = new StringBuilder(StatCollector.translateToLocal("tes.gui.map.conquest.noPledge"));
				} else {
					TESPlayerData pd = TESLevelData.getData(mc.thePlayer);
					TESFaction pledgeFac = pd.getPledgeFaction();
					TESFactionRank needRank = query.getNeedRank();
					String needAlign = TESAlignmentValues.formatAlignForDisplay(needRank.getAlignment());
					String format = "";
					if (query.getResult() == TESConquestGrid.ConquestViewable.NEED_RANK) {
						format = "tes.gui.map.conquest.needRank";
					}
		...
		if (!hasOverlay) {
			if (isGameOfThrones() && selectedWaypoint != null) {
				...
				if (selectedWaypoint instanceof TESCustomWaypoint) {
					TESCustomWaypoint cwp = (TESCustomWaypoint) selectedWaypoint;
					if (cwp.isShared()) {
						notUnlocked = StatCollector.translateToLocal("tes.gui.map.locked.shared");
					}
				} else {
					TESWaypoint wp = (TESWaypoint) selectedWaypoint;
					notUnlocked = wp.isCompatibleAlignment(mc.thePlayer) ? StatCollector.translateToLocal("tes.gui.map.locked.region") : StatCollector.translateToLocal("tes.gui.map.locked.enemy");
				}
				String conquestUnlock = pd.getPledgeFaction() == null ? "" : StatCollector.translateToLocalFormatted("tes.gui.map.locked.conquerable", pd.getPledgeFaction().factionName());
				String ftPrompt = StatCollector.translateToLocalFormatted("tes.gui.map.fastTravel.prompt", GameSettings.getKeyDisplayString(TESKeyHandler.KEY_BINDING_FAST_TRAVEL.getKeyCode()));
				String ftMoreTime = StatCollector.translateToLocalFormatted("tes.gui.map.fastTravel.moreTime", TESLevelData.getHMSTime_Ticks(timeRemaining));
				String ftWaitTime = StatCollector.translateToLocalFormatted("tes.gui.map.fastTravel.waitTime", TESLevelData.getHMSTime_Ticks(wpTimeThreshold));
				if (fullscreen || isConquestGrid) {
					if (!hasUnlocked) {
						if (selectedWaypoint instanceof TESWaypoint && ((TESWaypoint) selectedWaypoint).isConquestUnlockable(mc.thePlayer)) {
				...
				TESBiome biome = TESGenLayerWorld.getBiomeOrOcean(biomePosX_int, MathHelper.floor_double(biomePosZ));
				mouseXCoord = Math.round((biomePosX - TESGenLayerWorld.ORIGIN_X) * TESGenLayerWorld.SCALE);
				mouseZCoord = Math.round((biomePosZ - TESGenLayerWorld.ORIGIN_Z) * TESGenLayerWorld.SCALE);
				String biomeName = biome.getBiomeDisplayName();
				String coords = StatCollector.translateToLocalFormatted("tes.gui.map.coords", mouseXCoord, mouseZCoord);
				String teleport = StatCollector.translateToLocalFormatted("tes.gui.map.tp", GameSettings.getKeyDisplayString(TESKeyHandler.KEY_BINDING_MAP_TELEPORT.getKeyCode()));
				int stringHeight = fontRendererObj.FONT_HEIGHT;
				if (fullscreen || isConquestGrid) {
		...
		if (isConquestGrid) {
			s = StatCollector.translateToLocalFormatted("tes.gui.map.conquest.title", conquestViewingFaction.factionName());
			x = mapXMin + mapWidth / 2;
			y = mapYMin - 14;
			TESTickHandlerClient.drawAlignmentText(fontRendererObj, x - fontRendererObj.getStringWidth(s) / 2, y, s, 1.0f);
			if (!loadingConquestGrid) {
				int keyBorder = 8;
				int keyWidth = 24;
				int keyHeight = 12;
				...
				for (int pass = 0; pass <= 1; ++pass) {
					for (int l = 0; l <= 10; ++l) {
						...
						if (pass == 0) {
							...
							String keyLabel = TESAlignmentValues.formatConqForDisplay(strFrac, false);
		...
		if (!hasOverlay && hasControlZones) {
			s = StatCollector.translateToLocalFormatted("tes.gui.map.controlZones", controlZoneFaction.factionName());
			x = mapXMin + mapWidth / 2;
			y = mapYMin + 20;
			TESTickHandlerClient.drawAlignmentText(fontRendererObj, x - fontRendererObj.getStringWidth(s) / 2, y, s, 1.0f);
			if (!TESFaction.controlZonesEnabled(mc.theWorld)) {
				s = StatCollector.translateToLocal("tes.gui.map.controlZonesDisabled");
				TESTickHandlerClient.drawAlignmentText(fontRendererObj, x - fontRendererObj.getStringWidth(s) / 2, mapYMin + mapHeight / 2, s, 1.0f);
			}
		}
			...
			if (overlayLines != null) {
				...
				if (sharingWaypoint) {
					...
					for (int index = sharesMinMax[0]; index <= sharesMinMax[1]; ++index) {
						...
						TESFellowshipClient fs = TESLevelData.getData(mc.thePlayer).getClientFellowshipByID(fsID);
						if (fs != null) {
							...
							mc.getTextureManager().bindTexture(TESGuiFellowships.ICONS_TEXTURES);
							GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
				...
				if (creatingWaypointNew || renamingWaypoint || sharingWaypointNew) {
					...
					GL11.glPopMatrix();
					if (sharingWaypointNew && isFellowshipAlreadyShared(nameWPTextField.getText(), (TESCustomWaypoint) selectedWaypoint)) {
						String alreadyShared = StatCollector.translateToLocal("tes.gui.map.customWaypoint.shareNew.already");
				...
				if (buttonOverlayFunction.visible) {
					...
					if (sharingWaypointNew) {
						buttonOverlayFunction.enabled = isExistingUnsharedFellowshipName(nameWPTextField.getText(), (TESCustomWaypoint) selectedWaypoint);
					}
	...
	private TESFellowshipClient getFellowshipByName(String name) {
		String fsName = StringUtils.strip(name);
		return TESLevelData.getData(mc.thePlayer).getClientFellowshipByName(fsName);
	}
	...
	private boolean hasMapLabels() {
		if (isConquestGrid) {
			return TESConfig.mapLabelsConquest;
		}
		return TESConfig.mapLabels;
	}
	...
	public void initGui() {
		...
		if (isConquestGrid) {
			...
			setupMapDimensions();
			conquestViewingFaction = TESLevelData.getData(mc.thePlayer).getPledgeFaction();
			if (conquestViewingFaction == null) {
				conquestViewingFaction = TESLevelData.getData(mc.thePlayer).getViewingFaction();
			}
			...
			setCurrentScrollFromFaction();
			buttonConquestRegions = new TESGuiButton(200, mapXMax - 120, mapYMax + 5, 120, 20, "");
		...
		if (hasControlZones) {
			...
			posX = x / TESGenLayerWorld.SCALE + TESGenLayerWorld.ORIGIN_X;
			posY = z / TESGenLayerWorld.SCALE + TESGenLayerWorld.ORIGIN_Z;
			int zoneWidth = xMax - xMin;
			int zoneHeight = zMax - zMin;
			double mapZoneWidth = (double) zoneWidth / TESGenLayerWorld.SCALE;
			double mapZoneHeight = (double) zoneHeight / TESGenLayerWorld.SCALE;
			...
			posX = (float) (mc.thePlayer.posX / TESGenLayerWorld.SCALE) + TESGenLayerWorld.ORIGIN_X;
			posY = (float) (mc.thePlayer.posZ / TESGenLayerWorld.SCALE) + TESGenLayerWorld.ORIGIN_Z;
		}
		prevPosX = posX;
		prevPosY = posY;
		...
		fellowshipDrawGUI = new TESGuiFellowships();
		fellowshipDrawGUI.setWorldAndResolution(mc, width, height);
		if (mc.currentScreen == this) {
			IMessage packet = new TESPacketClientMQEvent(TESPacketClientMQEvent.ClientMQEvent.MAP);
			TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
		}
	}

	private boolean isExistingFellowshipName(String name) {
		TESFellowshipClient fs = getFellowshipByName(name);
		return fs != null;
	}

	private boolean isExistingUnsharedFellowshipName(String name, TESCustomWaypoint waypoint) {
		TESFellowshipClient fs = getFellowshipByName(name);
		return fs != null && !waypoint.hasSharedFellowship(fs.getFellowshipID());
	}

	private boolean isFellowshipAlreadyShared(String name, TESCustomWaypoint waypoint) {
		return isExistingFellowshipName(name) && !isExistingUnsharedFellowshipName(name, waypoint);
	}

	private boolean isGameOfThrones() {
		return mc.thePlayer.dimension == TESDimension.GAME_OF_THRONES.getDimensionID();
	}
	...
	public void keyTyped(char c, int i) {
		if (hasOverlay) {
			...
			if (!loadingConquestGrid) {
				TESPlayerData pd = TESLevelData.getData(mc.thePlayer);
				if (i == TESKeyHandler.KEY_BINDING_FAST_TRAVEL.getKeyCode() && isGameOfThrones() && selectedWaypoint != null && selectedWaypoint.hasPlayerUnlocked(mc.thePlayer) && pd.getTimeSinceFT() >= pd.getWaypointFTTime(selectedWaypoint, mc.thePlayer)) {
					IMessage packet = new TESPacketFastTravel(selectedWaypoint);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					mc.thePlayer.closeScreen();
					return;
				}
				if (selectedWaypoint == null && i == TESKeyHandler.KEY_BINDING_MAP_TELEPORT.getKeyCode() && isMouseWithinMap && canTeleport()) {
					IMessage packet = new TESPacketMapTp(mouseXCoord, mouseZCoord);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					mc.thePlayer.closeScreen();
					return;
				}
			...
			if (hasControlZones && (i == 1 || i == mc.gameSettings.keyBindInventory.getKeyCode())) {
				Minecraft.getMinecraft().displayGuiScreen(new TESGuiFactions());
				return;
			}
			super.keyTyped(c, i);
		}
	...
	public void mouseClicked(int i, int j, int k) {
		IMessage packet;
		TESGuiMapWidget mouseWidget = null;
		for (TESGuiMapWidget widget : MAP_WIDGETS) {
		...
		if (hasOverlay && k == 0 && sharingWaypoint && mouseOverRemoveSharedFellowship != null) {
			...
			packet = new TESPacketShareCWP(selectedWaypoint, fsName, false);
			TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
			return;
		}
		if (!hasOverlay && k == 0 && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint) {
			TESCustomWaypoint cwp = (TESCustomWaypoint) selectedWaypoint;
			if (cwp.isShared()) {
				if (mouseWidget == widgetHideSWP) {
					packet = new TESPacketCWPSharedHide(cwp, true);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					selectedWaypoint = null;
					return;
				}
				if (mouseWidget == widgetUnhideSWP) {
					packet = new TESPacketCWPSharedHide(cwp, false);
					TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
					return;
				}
			} else {
		...
		if (!hasOverlay && k == 0) {
			if (mouseWidget == widgetToggleWPs) {
				showWP = !showWP;
				TESClientProxy.sendClientInfoPacket(null, null);
				return;
			}
		...
		if (!hasOverlay && !loadingConquestGrid && k == 0 && isMouseWithinMap) {
			...
			List<TESAbstractWaypoint> waypoints = TESLevelData.getData(mc.thePlayer).getAllAvailableWaypoints();
			for (TESAbstractWaypoint waypoint : waypoints) {
				float dx;
				float dy;
				double distToWP;
				float[] pos;
	...
	private void openOverlayCreate() {
		...
		int numWaypoints = TESLevelData.getData(mc.thePlayer).getCustomWaypoints().size();
		int maxWaypoints = TESLevelData.getData(mc.thePlayer).getMaxCustomWaypoints();
		int remaining = maxWaypoints - numWaypoints;
		if (remaining <= 0) {
			overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.allUsed.1", maxWaypoints), "", StatCollector.translateToLocal("tes.gui.map.customWaypoint.allUsed.2")};
		} else {
			overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.create.1", numWaypoints, maxWaypoints), "", StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.create.2")};
			buttonOverlayFunction.visible = true;
			buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.create.button");
		}
	}
	...
	private void openOverlayCreateNew() {
		...
		overlayLines = new String[]{StatCollector.translateToLocal("tes.gui.map.customWaypoint.createNew.1")};
		buttonOverlayFunction.visible = true;
		buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.createNew.button");
	}
	...
	private void openOverlayDelete() {
		...
		overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.delete.1", selectedWaypoint.getDisplayName())};
		buttonOverlayFunction.visible = true;
		buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.delete.button");
	}
	...
	private void openOverlayRename() {
		...
		overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.rename.1", selectedWaypoint.getDisplayName())};
		buttonOverlayFunction.visible = true;
		buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.rename.button");
	}
	...
	private void openOverlayShare() {
		...
		overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.share.1", selectedWaypoint.getDisplayName())};
		buttonOverlayFunction.visible = true;
		buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.share.button");
	}
	...
	private void openOverlayShareNew() {
		...
		overlayLines = new String[]{StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.shareNew.1", selectedWaypoint.getDisplayName())};
		buttonOverlayFunction.visible = true;
		buttonOverlayFunction.displayString = StatCollector.translateToLocal("tes.gui.map.customWaypoint.shareNew.button");
	}
	...
	private void renderBeziers() {
		if (!showWP && !showCWP || !TESFixedStructures.hasMapFeatures(mc.theWorld)) {
			return;
		}
				...
				for (int i = 0; i < bezier.getBezierPoints().length; i += interval) {
					...
					if (labels && x >= mapXMin - (clip = 200) && x <= mapXMax + clip && y >= mapYMin - clip && y <= mapYMax + clip) {
						...
						if (i % nameInterval < interval) {
							...
							for (TESBeziers.BezierPoint end : bezier.getEndpoints()) {
								float dy;
							...
							if (!endNear && zoomlerp > 0.0f) {
								...
								TESBeziers.BezierPoint nextPoint = bezier.getBezierPoints()[Math.min(i + 1, bezier.getBezierPoints().length - 1)];
								TESBeziers.BezierPoint prevPoint = bezier.getBezierPoints()[Math.max(i - 1, 0)];
								...
								int alphaI = TESClientProxy.getAlphaInt(zoomlerp * 0.8f);
								GL11.glEnable(3042);
								GL11.glBlendFunc(770, 771);
	...
	private void renderControlZone(int mouseX, int mouseY) {
		List<TESControlZone> controlZones;
		mouseControlZone = false;
		mouseControlZoneReduced = false;
		TESFaction faction = controlZoneFaction;
		if (faction.getFactionDimension() == TESDimension.GAME_OF_THRONES && !(controlZones = faction.getControlZones()).isEmpty()) {
			Tessellator tessellator = Tessellator.instance;
			setupMapClipping();
			GL11.glDisable(3553);
			for (int pass = 0; pass <= 2; ++pass) {
				...
				for (TESControlZone zone : controlZones) {
					...
					float radiusWorld = TESWaypoint.mapToWorldR(radius);
	...
	private void renderLabels() {
		...
		setupMapClipping();
		for (TESMapLabels label : TESMapLabels.allMapLabels()) {
	...
	public void renderMapAndOverlay(boolean sepia, float alpha, boolean overlay) {
		...
		double minU = (double) (posX - mapScaleX / 2.0f) / TESGenLayerWorld.getImageWidth();
		double maxU = (double) (posX + mapScaleX / 2.0f) / TESGenLayerWorld.getImageWidth();
		double minV = (double) (posY - mapScaleY / 2.0f) / TESGenLayerWorld.getImageHeight();
		double maxV = (double) (posY + mapScaleY / 2.0f) / TESGenLayerWorld.getImageHeight();
		if (minU < 0.0) {
			mapXMin_W = mapXMin + (int) Math.round((0.0 - minU) * TESGenLayerWorld.getImageWidth() * zoomScale);
			minU = 0.0;
			if (maxU < 0.0) {
				maxU = 0.0;
				mapXMax_W = mapXMin_W;
		...
		if (maxU > 1.0) {
			mapXMax_W = mapXMax - (int) Math.round((maxU - 1.0) * TESGenLayerWorld.getImageWidth() * zoomScale);
			maxU = 1.0;
			if (minU > 1.0) {
				minU = 1.0;
				mapXMin_W = mapXMax_W;
		...
		if (minV < 0.0) {
			mapYMin_W = mapYMin + (int) Math.round((0.0 - minV) * TESGenLayerWorld.getImageHeight() * zoomScale);
			minV = 0.0;
			if (maxV < 0.0) {
				maxV = 0.0;
				mapYMax_W = mapYMin_W;
		...
		if (maxV > 1.0) {
			mapYMax_W = mapYMax - (int) Math.round((maxV - 1.0) * TESGenLayerWorld.getImageHeight() * zoomScale);
			maxV = 1.0;
			if (minV > 1.0) {
				minV = 1.0;
				mapYMin_W = mapYMax_W;
		...
		TESTextures.drawMap(sepia, mapXMin_W, mapXMax_W, mapYMin_W, mapYMax_W, zLevel, minU, maxU, minV, maxV, alpha);
		if (overlay && !isOSRS()) {
			TESTextures.drawMapOverlay(mapXMin, mapXMax, mapYMin, mapYMax, zLevel);
		}
	}
	...
	private void renderMapWidgets(int mouseX, int mouseY) {
		...
		widgetDelCWP.setVisible(!hasOverlay && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint && !((TESCustomWaypoint) selectedWaypoint).isShared());
		widgetRenameCWP.setVisible(!hasOverlay && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint && !((TESCustomWaypoint) selectedWaypoint).isShared());
		...
		widgetShareCWP.setVisible(!hasOverlay && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint && !((TESCustomWaypoint) selectedWaypoint).isShared());
		widgetHideSWP.setVisible(!hasOverlay && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint && ((TESCustomWaypoint) selectedWaypoint).isShared() && !((TESCustomWaypoint) selectedWaypoint).isSharedHidden());
		widgetUnhideSWP.setVisible(!hasOverlay && isGameOfThrones() && selectedWaypoint instanceof TESCustomWaypoint && ((TESCustomWaypoint) selectedWaypoint).isShared() && ((TESCustomWaypoint) selectedWaypoint).isSharedHidden());
		TESGuiMapWidget mouseOverWidget = null;
		for (TESGuiMapWidget widget : MAP_WIDGETS) {
			if (widget.isVisible()) {
	...
	private void renderMiniQuests(EntityPlayer entityplayer, int mouseX, int mouseY) {
		...
		TESMiniQuest mouseOverQuest = null;
		int mouseOverQuestX = 0;
		int mouseOverQuestY = 0;
		double distanceMouseOverQuest = Double.MAX_VALUE;
		List<TESMiniQuest> quests = TESLevelData.getData(entityplayer).getActiveMiniQuests();
		for (TESMiniQuest quest : quests) {
	...
	private void renderWaypoints(int pass, int mouseX, int mouseY) {
		if (!showWP && !showCWP && !showHiddenSWP || !TESFixedStructures.hasMapFeatures(mc.theWorld)) {
			return;
		}
		renderWaypoints(TESLevelData.getData(mc.thePlayer).getAllAvailableWaypoints(), pass, mouseX, mouseY, hasMapLabels(), false);
	}

	public void renderWaypoints(Iterable<TESAbstractWaypoint> waypoints, int pass, int mouseX, int mouseY, boolean labels, boolean overrideToggles) {
		setupMapClipping();
		TESAbstractWaypoint mouseOverWP = null;
					...
					if (x >= mapXMin - clip && x <= mapXMax + clip && y >= mapYMin - clip && y <= mapYMax + clip) {
						if (pass == 0) {
							...
							if (isOSRS()) {
								...
								mc.getTextureManager().bindTexture(TESTextures.OSRS_TEXTURE);
								GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
								drawTexturedModalRectFloat(x / 0.33F - 8.0F, y / 0.33F - 8.0F, 0, 0, 15.0F, 15.0F);
								GL11.glPopMatrix();
							} else {
								TESAbstractWaypoint.WaypointLockState state = mc.thePlayer != null ? waypoint.getLockState(mc.thePlayer) : TESAbstractWaypoint.WaypointLockState.STANDARD_UNLOCKED;
								mc.getTextureManager().bindTexture(MAP_ICONS_TEXTURE);
	...
	private void renderWaypointTooltip(TESAbstractWaypoint waypoint, boolean selected) {
		String name = waypoint.getDisplayName();
		int wpX = waypoint.getCoordX();
		int wpZ = waypoint.getCoordZ();
		int wpY = waypoint.getCoordYSaved();
		String coords = wpY >= 0 ? StatCollector.translateToLocalFormatted("tes.gui.map.coordsY", wpX, wpY, wpZ) : StatCollector.translateToLocalFormatted("tes.gui.map.coords", wpX, wpZ);
	...
	private void requestConquestGridIfNeed(TESFaction conqFac) {
		if (!requestedFacGrids.contains(conqFac) && ticksUntilRequestFac <= 0) {
			requestedFacGrids.add(conqFac);
			IMessage packet = new TESPacketConquestGridRequest(conqFac);
			TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
		}
	}
	...
	private void requestIsOp() {
		if (mc.isSingleplayer()) {
			...
			IMessage packet = new TESPacketIsOpRequest();
			TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
		}
	}

	public TESGuiMap setConquestGrid() {
		isConquestGrid = true;
		return this;
	}

	public void setControlZone(TESFaction f) {
	...
	public void setCWPProtectionMessage(IChatComponent message) {
		...
		creatingWaypoint = false;
		creatingWaypointNew = false;
		String protection = StatCollector.translateToLocalFormatted("tes.gui.map.customWaypoint.protected.1", message.getFormattedText());
		String warning = StatCollector.translateToLocal("tes.gui.map.customWaypoint.protected.2");
	...
	private void setupMapWidgets() {
		MAP_WIDGETS.clear();
		widgetAddCWP = new TESGuiMapWidget(-16, 6, 10, "addCWP", 0, 0);
		MAP_WIDGETS.add(widgetAddCWP);
		widgetDelCWP = new TESGuiMapWidget(-16, 20, 10, "delCWP", 10, 0);
		MAP_WIDGETS.add(widgetDelCWP);
		widgetRenameCWP = new TESGuiMapWidget(-16, 34, 10, "renameCWP", 0, 10);
		MAP_WIDGETS.add(widgetRenameCWP);
		widgetToggleWPs = new TESGuiMapWidget(-58, 6, 10, "toggleWPs", 80, 0);
		MAP_WIDGETS.add(widgetToggleWPs);
		widgetToggleCWPs = new TESGuiMapWidget(-44, 6, 10, "toggleCWPs", 90, 0);
		MAP_WIDGETS.add(widgetToggleCWPs);
		widgetToggleHiddenSWPs = new TESGuiMapWidget(-30, 6, 10, "toggleHiddenSWPs", 100, 0);
		MAP_WIDGETS.add(widgetToggleHiddenSWPs);
		widgetZoomIn = new TESGuiMapWidget(6, 6, 10, "zoomIn", 30, 0);
		MAP_WIDGETS.add(widgetZoomIn);
		widgetZoomOut = new TESGuiMapWidget(6, 20, 10, "zoomOut", 40, 0);
		MAP_WIDGETS.add(widgetZoomOut);
		widgetFullScreen = new TESGuiMapWidget(20, 6, 10, "fullScreen", 50, 0);
		MAP_WIDGETS.add(widgetFullScreen);
		widgetSepia = new TESGuiMapWidget(34, 6, 10, "sepia", 60, 0);
		MAP_WIDGETS.add(widgetSepia);
		widgetLabels = new TESGuiMapWidget(-72, 6, 10, "labels", 70, 0);
		MAP_WIDGETS.add(widgetLabels);
		widgetShareCWP = new TESGuiMapWidget(-16, 48, 10, "shareCWP", 10, 10);
		MAP_WIDGETS.add(widgetShareCWP);
		widgetHideSWP = new TESGuiMapWidget(-16, 20, 10, "hideSWP", 20, 0);
		MAP_WIDGETS.add(widgetHideSWP);
		widgetUnhideSWP = new TESGuiMapWidget(-16, 20, 10, "unhideSWP", 20, 10);
	...
	private void setupScrollBars(int i, int j) {
		...
		if (selectedWaypoint != null && hasOverlay && sharingWaypoint) {
			displayedWPShareList = ((TESCustomWaypoint) selectedWaypoint).getSharedFellowshipIDs();
	...
	private void toggleMapLabels() {
		if (isConquestGrid) {
			TESConfig.toggleMapLabelsConquest();
		} else {
			TESConfig.toggleMapLabels();
		}
	}
	...
	private float[] transformCoords(float x, float z) {
		float x1 = x / TESGenLayerWorld.SCALE + TESGenLayerWorld.ORIGIN_X;
		float z1 = z / TESGenLayerWorld.SCALE + TESGenLayerWorld.ORIGIN_Z;
