...
public class TESGuiQuestBook extends TESGuiScreenBase {
	public static final ResourceLocation GUI_TEXTURE = new ResourceLocation("tes:textures/gui/quest/questBook.png");

	private static final ResourceLocation GUI_TEXTURE_MINIQUESTS = new ResourceLocation("tes:textures/gui/quest/questBook_miniquests.png");
	...
	private final Map<TESMiniQuest, Pair<Integer, Integer>> displayedMiniQuests = new HashMap<>();

	private TESMiniQuest deletingMiniquest;
	private TESMiniQuest selectedMiniquest;
	...
	public void actionPerformed(GuiButton button) {
		if (button.enabled) {
			...
			if (button == buttonQuestDelete && deletingMiniquest != null) {
				IMessage packet = new TESPacketDeleteMiniquest(deletingMiniquest);
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (page == Page.MINIQUESTS && selectedMiniquest == null) {
			...
			y = guiTop + 50;
			if (viewCompleted) {
				drawCenteredString(StatCollector.translateToLocal("tes.gui.redBook.mq.viewComplete"), x, y, 8019267);
			} else {
				drawCenteredString(StatCollector.translateToLocal("tes.gui.redBook.mq.viewActive"), x, y, 8019267);
			}
		}
		if (page == Page.MINIQUESTS) {
			if (selectedMiniquest == null) {
				drawCenteredString(TESDate.AegonCalendar.getDate().getDateName(false), guiLeft + X_SIZE / 2 - PAGE_BORDER - PAGE_WIDTH / 2, guiTop + Y_SIZE - 30, 8019267);
				drawCenteredString(StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.numActive", getPlayerData().getActiveMiniQuests().size()), x, guiTop + 120, 8019267);
				drawCenteredString(StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.numComplete", getPlayerData().getCompletedMiniQuestsTotal()), x, guiTop + 140, 8019267);
			} else {
				TESMiniQuest quest = selectedMiniquest;
				mc.getTextureManager().bindTexture(GUI_TEXTURE);
				...
				TESTextBody pageText = new TESTextBody(8019267);
				pageText.setTextWidth(textW);
				String[] dayYear = TESDate.AegonCalendar.getDate(quest.getDateGiven()).getDayAndYearNames(false);
				...
				pageText.add("");
				String startQuote = TESSpeech.formatSpeech(quest.getQuoteStart(), mc.thePlayer, null, quest.getObjectiveInSpeech());
				startQuote = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.quote", startQuote);
				pageText.add(startQuote);
				pageText.add("");
				List<String> quotesStages = quest.getQuotesStages();
				if (!quotesStages.isEmpty()) {
					for (String s : quotesStages) {
						String stageQuote = TESSpeech.formatSpeech(s, mc.thePlayer, null, quest.getObjectiveInSpeech());
						stageQuote = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.quote", stageQuote);
						pageText.add(stageQuote);
						pageText.add("");
					}
				}
				String asked = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.asked", entityName, quest.getQuestObjective());
				pageText.add(asked);
				pageText.add("");
				String progress = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.progress", quest.getQuestProgress());
				pageText.add(progress);
				if (quest.isWillHire()) {
					pageText.add("");
					String willHire = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.willHire", entityName);
				...
				if (completed) {
					...
					pageText.add("");
					dayYear = TESDate.AegonCalendar.getDate(quest.getDateCompleted()).getDayAndYearNames(false);
					pageText.add(dayYear[0]);
					pageText.add(dayYear[1]);
					pageText.add("");
					String completeQuote = TESSpeech.formatSpeech(quest.getQuoteComplete(), mc.thePlayer, null, quest.getObjectiveInSpeech());
					completeQuote = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.quote", completeQuote);
					pageText.add(completeQuote);
					pageText.add("");
					String completedText = StatCollector.translateToLocal("tes.gui.redBook.mq.diary.complete");
					pageText.add(completedText);
					if (quest.anyRewardsGiven()) {
						pageText.add("");
						String rewardText = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward", entityName);
						pageText.add(rewardText);
						if (quest.getAlignmentRewarded() != 0.0f) {
							String alignS = TESAlignmentValues.formatAlignForDisplay(quest.getAlignmentRewarded());
							String rewardAlign = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward.align", alignS, factionName);
							pageText.add(rewardAlign);
						}
						if (quest.getCoinsRewarded() != 0.0f) {
							String rewardCoins = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward.coins", quest.getCoinsRewarded());
							pageText.add(rewardCoins);
						}
						if (!quest.getItemsRewarded().isEmpty()) {
							for (ItemStack item : quest.getItemsRewarded()) {
								String rewardItem = item.getItem() instanceof ItemEditableBook ? StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward.book", item.getDisplayName()) : StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward.item", item.getDisplayName(), item.stackSize);
					...
					if (quest.isWasHired()) {
						pageText.add("");
						String rewardHired = StatCollector.translateToLocalFormatted("tes.gui.redBook.mq.diary.reward.hired", entityName);
			...
			if (deletingMiniquest == null) {
				List<TESMiniQuest> miniquests = getMiniQuests();
				if (!(miniquests = new ArrayList<>(miniquests)).isEmpty()) {
					if (viewCompleted) {
						miniquests = Lists.reverse(miniquests);
					} else {
						miniquests.sort(new TESMiniQuest.SorterAlphabetical());
					}
					...
					for (int index = min; index <= max; ++index) {
						TESMiniQuest quest = miniquests.get(index);
				...
				String deleteText = viewCompleted ? StatCollector.translateToLocal("tes.gui.redBook.mq.deleteCmp") : StatCollector.translateToLocal("tes.gui.redBook.mq.delete");
		...
		if (viewCompleted) {
			buttonQuestDelete.displayString = StatCollector.translateToLocal("tes.gui.redBook.mq.deleteCmpYes");
			buttonQuestDeleteCancel.displayString = StatCollector.translateToLocal("tes.gui.redBook.mq.deleteCmpNo");
		} else {
			buttonQuestDelete.displayString = StatCollector.translateToLocal("tes.gui.redBook.mq.deleteYes");
			buttonQuestDeleteCancel.displayString = StatCollector.translateToLocal("tes.gui.redBook.mq.deleteNo");
		}
		super.drawScreen(i, j, f);
	}

	private List<TESMiniQuest> getMiniQuests() {
		if (viewCompleted) {
	...
	private TESPlayerData getPlayerData() {
		return TESLevelData.getData(mc.thePlayer);
	}
	...
	public void initGui() {
		...
		buttonViewActive = new TESGuiButton(2, buttonX - 10 - 60, buttonY, 60, 20, StatCollector.translateToLocal("tes.gui.redBook.mq.viewActive"));
		buttonList.add(buttonViewActive);
		buttonViewCompleted = new TESGuiButton(3, buttonX + 10, buttonY, 60, 20, StatCollector.translateToLocal("tes.gui.redBook.mq.viewComplete"));
		buttonList.add(buttonViewCompleted);
		buttonX = guiLeft + X_SIZE / 2 + PAGE_BORDER + PAGE_WIDTH / 2;
		buttonY = guiTop + Y_SIZE - 60;
		buttonQuestDelete = new TESGuiButton(2, buttonX - 10 - 60, buttonY, 60, 20, "");
		buttonList.add(buttonQuestDelete);
		buttonQuestDeleteCancel = new TESGuiButton(3, buttonX + 10, buttonY, 60, 20, "");
	...
	public void mouseClicked(int i, int j, int mouse) {
		if (mouse == 0) {
			...
			if (page == Page.MINIQUESTS && deletingMiniquest == null) {
				for (Map.Entry<TESMiniQuest, Pair<Integer, Integer>> entry : displayedMiniQuests.entrySet()) {
					quest = entry.getKey();
	...
	private void renderMiniQuestPanel(TESMiniQuest quest, int questX, int questY, int mouseX, int mouseY) {
		GL11.glPushMatrix();
		...
		if (quest.isFailed()) {
			...
			fontRendererObj.drawString(StatCollector.translateToLocal("tes.gui.redBook.mq.tracking"), questX + qPanelBorder, questY + 25, 8019267);
		} else {
			...
			if (quest.isCompleted()) {
				progress = StatCollector.translateToLocal("tes.gui.redBook.mq.complete");
			}
	...
	private void trackOrUntrack(TESMiniQuest quest) {
		TESMiniQuest tracking = getPlayerData().getTrackingMiniQuest();
		TESMiniQuest newTracking = quest == tracking ? null : quest;
		IMessage packet = new TESPacketMiniquestTrack(newTracking);
		TESPacketHandler.NETWORK_WRAPPER.sendToServer(packet);
	...
	private enum Page {
		...
		private String getTitle() {
			return StatCollector.translateToLocal("tes.gui.redBook.page." + name);
		}
	}
}