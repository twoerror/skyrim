...
public class TESGuiNotificationDisplay extends Gui {
	public static final TESGuiNotificationDisplay INSTANCE = new TESGuiNotificationDisplay();
	...
	private TESGuiNotificationDisplay() {
	}
	...
	public void queueConquest(TESFaction fac, float conq, boolean cleansing) {
			...
			for (Notification notif : notifications) {
				...
				if (d0 < 0.0 || d0 > 1.0) {
					...
					if (Minecraft.isGuiEnabled()) {
						...
						GL11.glEnable(3553);
						MINECRAFT.getTextureManager().bindTexture(TESGuiAchievements.ICONS_TEXTURE);
						GL11.glDisable(2896);
						int guiYSize = 32;
	...
	private class NotificationAchievement extends Notification {
		private final TESAchievement achievement;

		private NotificationAchievement(TESAchievement ach) {
			achievement = ach;
			durationMs = 3000;
		}
	...
	private class NotificationConquest extends Notification {
		private final TESFaction conqFac;
		private final float conqValue;
		private final boolean isCleansing;

		private NotificationConquest(TESFaction fac, float conq, boolean clean) {
			conqFac = fac;
			conqValue = conq;
			isCleansing = clean;
			durationMs = 6000;
		...
		@Override
		public void renderText(int x, int y) {
			String conqS = TESAlignmentValues.formatConqForDisplay(conqValue, false);
			TESTickHandlerClient.drawConquestText(MINECRAFT.fontRenderer, x + 1, y, conqS, isCleansing, 1.0f);
			MINECRAFT.fontRenderer.drawString(StatCollector.translateToLocal("tes.gui.map.conquest.notif"), x + MINECRAFT.fontRenderer.getStringWidth(conqS + ' ') + 2, y, 8019267);
	...
	private class NotificationFellowship extends Notification {
		...
		@Override
		public void renderIcon(int x, int y) {
			MINECRAFT.getTextureManager().bindTexture(TESGuiFellowships.ICONS_TEXTURES);
			GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
