...
public class TESGuiAchievements extends TESGuiMenuBaseReturn {
	public static final ResourceLocation ICONS_TEXTURE = new ResourceLocation("tes:textures/gui/achievements/icons.png");

	private static final ResourceLocation PAGE_TEXTURE = new ResourceLocation("tes:textures/gui/achievements/page.png");

	private static TESDimension currentDimension;
	private static TESDimension prevDimension;

	private static TESAchievement.Category currentCategory;

	private final List<TESAchievement> currentCategoryTakenAchievements = new ArrayList<>();
	private final List<TESAchievement> currentCategoryUntakenAchievements = new ArrayList<>();

	private TESGuiButtonAchievements buttonCategoryPrev;
	private TESGuiButtonAchievements buttonCategoryNext;
	...
	private static TESAchievement.Category getCategoryAtRelativeIndex(int i) {
		List<TESAchievement.Category> categories = currentDimension.getAchievementCategories();
	...
	public void drawScreen(int i, int j, float f) {
		...
		categoryName = StatCollector.translateToLocalFormatted("tes.gui.achievements.category", categoryName, currentCategoryTakenCount, currentCategoryTakenCount + currentCategoryUntakenCount);
		...
		for (int l = -catsEitherSide; l <= catsEitherSide; ++l) {
			...
			TESAchievement.Category thisCategory = getCategoryAtRelativeIndex(l);
	...
	public void initGui() {
		sizeX = 220;
		super.initGui();
		buttonCategoryPrev = new TESGuiButtonAchievements(0, true, guiLeft + 14, guiTop + 13);
		buttonList.add(buttonCategoryPrev);
		buttonCategoryNext = new TESGuiButtonAchievements(1, false, guiLeft + 191, guiTop + 13);
	...
	private void updateAchievementLists() {
		...
		for (TESAchievement achievement : currentCategory.getList()) {
			if (achievement.canPlayerEarn(mc.thePlayer)) {
				if (TESLevelData.getData(mc.thePlayer).hasAchievement(achievement)) {
		...
		Comparator<TESAchievement> sorter = TESAchievement.sortForDisplay(mc.thePlayer);
