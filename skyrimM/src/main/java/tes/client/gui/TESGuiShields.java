...
import tes.client.render.other.TESRenderShield;
import tes.common.TESLevelData;
import tes.common.database.TESShields;
import tes.common.network.TESPacketHandler;
import tes.common.network.TESPacketSelectShield;
...
public class TESGuiShields extends TESGuiMenuBase {
	...
	private TESShields.ShieldType currentShieldType;
	private TESShields currentShield;
	...
	private boolean canGoLeft() {
		for (int i = 0; i <= currentShieldID - 1; ++i) {
			TESShields shield = currentShieldType.getShields().get(i);
	...
	public void drawScreen(int i, int j, float f) {
		...
		TESRenderShield.renderShield(currentShield, null, PLAYER_MODEL);
		...
		shieldSelect.displayString = getPlayerEquippedShield() == currentShield ? StatCollector.translateToLocal("tes.gui.shields.selected") : StatCollector.translateToLocal("tes.gui.shields.select");
	...
	private TESShields getPlayerEquippedShield() {
		return TESLevelData.getData(mc.thePlayer).getShield();
	}
	...
	public void initGui() {
		...
		modelY = guiTop + 40;
		shieldLeft = new TESGuiButtonArrows(0, true, guiLeft + sizeX / 2 - 64, guiTop + 207);
		buttonList.add(shieldLeft);
		shieldSelect = new TESGuiButton(1, guiLeft + sizeX / 2 - 40, guiTop + 195, 80, 20, StatCollector.translateToLocal("tes.gui.shields.select"));
		buttonList.add(shieldSelect);
		shieldRight = new TESGuiButtonArrows(2, false, guiLeft + sizeX / 2 + 44, guiTop + 207);
		buttonList.add(shieldRight);
		shieldRemove = new TESGuiButton(3, guiLeft + sizeX / 2 - 40, guiTop + 219, 80, 20, StatCollector.translateToLocal("tes.gui.shields.remove"));
		...
		goToCape = new TESGuiButton(6, guiLeft + sizeX / 2 - 290, guiTop + 120, 160, 20, StatCollector.translateToLocal("tes.gui.capes"));
		buttonList.add(goToCape);
		TESShields equippedShield = getPlayerEquippedShield();
	...
	private void updateCurrentShield(int shield, int type) {
		...
		if (type != 0) {
			...
			if (currentShieldTypeID > TESShields.ShieldType.values().length - 1) {
				currentShieldTypeID = 0;
			}
			if (currentShieldTypeID < 0) {
				currentShieldTypeID = TESShields.ShieldType.values().length - 1;
			}
			currentShieldID = 0;
		}
		currentShieldType = TESShields.ShieldType.values()[currentShieldTypeID];
