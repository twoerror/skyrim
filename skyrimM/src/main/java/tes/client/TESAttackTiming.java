...
public class TESAttackTiming {
	private static final ResourceLocation METER_TEXTURE = new ResourceLocation("tes:textures/gui/attackMeter.png");
	...
	private TESAttackTiming() {
	}

	@SuppressWarnings("StatementWithEmptyBody")
	public static void doAttackTiming() {
		int currentTick = TESTickHandlerClient.getClientTick();
		if (lastCheckTick == -1) {
		...
		if (MC.thePlayer == null) {
			...
			if (pressed && MC.objectMouseOver != null && MC.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY && MC.objectMouseOver.entityHit instanceof EntityLivingBase) {
				if (attackTime > 0) {
					...
					attackTime = fullAttackTime = TESWeaponStats.getAttackTimePlayer(itemstack);
